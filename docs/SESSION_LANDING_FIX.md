# SESSION — Landing Page Fix
**Branch:** dev
**Goal:** Fix the broken landing page — it renders as unstyled HTML.
**Time estimate:** 30 minutes maximum.
...
cat docs/DESIGN_REFERENCE.html | head -100
```

Then read the current broken files:
```bash
cat src/app/HomeClient.tsx
cat src/components/landing/NavBar.tsx
cat src/components/landing/JourneyWizard.tsx
cat src/components/landing/FeatureGrid.tsx
```
...
```bash
# 1. Does the page compile?
npm run build 2>&1 | grep -iE "error|warning" | head -20

# 2. Are Tailwind classes being purged?
cat tailwind.config.ts | grep -A 10 "content"

# 3. Does globals.css import Tailwind?
head -10 src/app/globals.css

# 4. Check what the broken triangle is
grep -n "triangle\|▼\|▲\|clip-path\|polygon\|svg" src/components/landing/NavBar.tsx src/components/landing/JourneyWizard.tsx src/app/HomeClient.tsx | head -20
```
...
The landing page renders as unstyled HTML with a black triangle
at the top. This means one of three things:

A) Tailwind content paths don't include the new component files
B) A component is crashing silently and falling back to raw HTML
C) The new component files have a syntax error breaking hydration

The diagnosis commands above will tell you which one.

---

## THE FIX

### If problem is A — Tailwind not scanning new files:
Check tailwind.config.ts content array includes:
'./src/components/**/*.{js,ts,jsx,tsx}'

If missing, add it. Then rebuild.

### If problem is B or C — Component crash:

Do NOT try to fix the existing components.
Delete them and rewrite HomeClient.tsx as a single
self-contained file with NO external component imports.

```bash
rm src/components/landing/NavBar.tsx
rm src/components/landing/JourneyWizard.tsx
rm src/components/landing/FeatureGrid.tsx
```

Then rewrite src/app/HomeClient.tsx as one complete
self-contained file. No imports from landing components.
Everything inline in HomeClient.tsx.

---

## HOMELIENT.TSX — COMPLETE REWRITE IF NEEDED

If you need to rewrite, here is the exact structure.
Write this as one file. No external component dependencies.
...
Must be clean. Then restart dev server:
```bash
pkill -f "next dev" || true && sleep 2 && npm run dev &
```

Wait 8 seconds then check localhost:3000. Confirm:
- Dark background #0a0a0a on entire page
- Gold #C9A84C on logo, buttons, accents
- No black triangles or broken SVGs
- Nav has hamburger on mobile
- Journey wizard dropdowns work
...
git add src/components/landing/ 2>/dev/null || true
git commit -m "fix: landing page — self-contained HomeClient, Obsidian Gold, mobile-first"
git push origin dev
```

Update BUILD_TRACKER.md then:
```bash
git add BUILD_TRACKER.md
git commit -m "docs: BUILD_TRACKER — landing page fix complete"
git push origin dev
```
...
Report:
- What was broken (diagnosis result)
- Whether HomeClient was rewritten or patched
- Build status
- Commit hash
- Visual confirmation: dark bg / gold accents / no broken elements
