...
| docs/SPRINT_EM_EMAIL_AUDIT.md | Sprint EM execution contract — full 22-email inventory (content, trigger, purpose, file:line) grouped by live/gated/orphaned, plus tasks EM-1…EM-4 to activate the 7 dead ones (Clock-1 inactivity ×4, Clock-2 post-outcome ×3). | Rules, product spec |
...
| docs/DELIVERY_RELIABILITY_GAPS.md | Delivery-reliability gap register — the traced payment→generation→assembly→download failure path, gaps G-01…G-12 with severity, root cause, file:line evidence and status, what the client actually receives (ZIP manifest + typography), and the fee-vs-delivery reconciliation. Owns *what is broken* in delivery. | Fix sequencing, task status, document quality assessment |
| docs/SPRINT_DR_DELIVERY_RELIABILITY.md | Sprint DR execution contract — tasks DR-1…DR-22 across 8 phases, each with kind, estimate, exit criterion and named test, plus the effort table and the items blocked on Romy. Owns *what we do about the gaps, in what order, and how we prove it*. | Gap descriptions and evidence, rules, product spec |
| docs/RELIABILITY_SECURITY_GAPS.md | Reliability & security gap register outside the delivery path — gaps G-13…G-25 (payment/webhook, middleware access control, open redirects, error/404 pages, data retention, accessibility) with severity, file:line evidence, proof steps and a "verified sound" section. Companion to DELIVERY_RELIABILITY_GAPS.md, which owns the pipeline; this owns everything around it. | Fix sequencing/task status, rules, product spec |
| docs/SPRINT_RS_RELIABILITY_SECURITY.md | Sprint RS execution contract — tasks RS-1…RS-13 fixing G-13…G-25, each with kind, estimate, exit criterion and named test. Owns *what we do about the reliability/security gaps, in what order, and how we prove it*. | Gap descriptions and evidence, rules, product spec |
