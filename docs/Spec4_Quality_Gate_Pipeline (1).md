...
CONSISTENCY_FIELDS = {
    ...
    "total_business_cost_usd": "exact match required",
    "llc_name": "exact match required",
    "business_legal_name": "exact match required",
    ...
    "year1_revenue_projection": "exact match required",
    "year1_employee_count": "exact match required",
    "year3_revenue_projection": "exact match required",
...
`CONSISTENCY_FIELDS` is a list of NAMED, SPECIFIC values that should
be the SAME number/string everywhere they appear. This check does
NOT extract every dollar amount that appears anywhere in a document's
prose and compare it against `investment_amount_usd` or any other
total. A document legitimately contains many dollar figures that are
NOT `investment_amount_usd` and have no reason to match it — line
items in an investment breakdown (e.g. "$48,000 — leasehold
improvements"), individual fund sources (e.g. "$110,000 in personal
savings"), or revenue projections for years other than Year 1/3. None
of these are inconsistencies. If `extract_field_from_all_docs` for
`investment_amount_usd` is implemented by scanning for "any dollar
figure," it is implemented incorrectly — it must extract the SPECIFIC
value the document states AS the total investment (e.g. the figure
following "I have invested" / "total investment of" / the line marked
"TOTAL:" in a breakdown table), not every number that happens to
appear. See RULE 3A in CLAUDE_CONTEXT.md.

### Timeline Validator

In addition to the field-equality checks above, build a chronological
event map from every date field in `CONSISTENCY_FIELDS` plus any other
dated answers (lease signing, bank account opening, FDD review,
DS-160 submission, interview date) and flag sequences that are
chronologically impossible:

```
llc_formation_date AFTER lease_signing_date       → IMPOSSIBLE
wire_transfer_date BEFORE bank_account_opened_date → IMPOSSIBLE
fdd_review_date AFTER franchise_agreement_date     → IMPOSSIBLE
interview_date BEFORE ds160_submitted_date         → IMPOSSIBLE
```

An impossible sequence is a HARD failure — treat it the same as a
CONSISTENCY_FIELDS mismatch (see Handling Inconsistencies below). A
sequence that is merely unusual but not impossible (e.g. wire transfer
and LLC formation on the same day) is NOT a failure.

### Handling Inconsistencies

If any inconsistency or impossible timeline sequence is found:
- Do not advance to Stage 5
- Identify the SPECIFIC document(s) whose value differs from the
  majority/correct value — not all documents
- Return ONLY that specific document to Stage 1 (Generate) with an
  exact correction instruction stating both the wrong value found and
  the correct value to use, e.g.: "investment_amount_usd in
  source_of_funds is stated as $175,000; the correct value confirmed
  for this application is $185,000 — regenerate using $185,000."
- Never regenerate the entire application for a single mismatch
- Auto-correct ONLY when the discrepancy is purely cosmetic and
  unambiguous (e.g. "$185,000" vs "$185,000.00" vs "one hundred
  eighty-five thousand dollars" — same value, different formatting).
  A different NUMBER is never a cosmetic difference, regardless of
  how close the values are.
- If the regenerated document's new value still doesn't match
  CONSISTENCY_FIELDS after correction — this is itself a signal
  worth surfacing (see RULE 3A, Type 0/1/2) rather than retrying
  silently again
- Log the correction (document, field, old value, new value, instruction
  given) to `generation_pipeline_log`
