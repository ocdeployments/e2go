...
| **BC-8** | Make identity-document rejection classify by file content (magic bytes / structure), not the client-declared `type_${key}` field | G-4 | code | 1.5 days | DONE 2026-09-12 |
...
| # | Task | Gap | Kind | Effort | Status |
|---|---|---|---|---|---|
| **BC-13** | Proxy login/signup through an app API route so Turnstile + the Upstash rate limiter actually see auth attempts; make CAPTCHA fail closed, not open, on a network error | G-12 | code | 1.5 days | DONE 2026-09-12 — [login/route.ts](../src/app/api/auth/login/route.ts), [signup/route.ts](../src/app/api/auth/signup/route.ts) proxy both flows server-side through `checkRateLimit()` + `verifyTurnstile()` before ever touching Supabase Auth; [turnstile.ts](../src/lib/turnstile.ts) blocks (fail closed) on a genuine network/timeout error and only skips when the secret key is entirely unconfigured. [login/page.tsx](../src/app/login/page.tsx) rewired to call `/api/auth/login` (previously called `signInWithPassword()` directly, bypassing both controls) and given the same Turnstile widget/guard pattern signup already had. Along the way, found and fixed a pre-existing, unrelated bug also in scope of this gap: `next.config.mjs`'s CSP `script-src` was missing `challenges.cloudflare.com` and had no `frame-src` at all, so Turnstile's script silently failed to load everywhere it's configured (including the pre-existing signup page) — leaving a dead, invisible widget div that intercepted clicks on the submit button, making login/signup unusable. Fixed by adding `challenges.cloudflare.com` to `script-src`/`connect-src` and adding `frame-src` for its iframe. Verified live: `tsc`/`jest` (705)/`build` clean; CSP header confirmed correct via `fetch('/login')` from the running dev server; submit button confirmed `disabled` client-side while no CAPTCHA token is present; `curl` against `/api/auth/login` and `/api/auth/signup` confirmed both return 400 "Missing CAPTCHA token" with none supplied and 400 "CAPTCHA verification failed" with an invalid one (fail-closed, not skipped); 9 rapid requests to `/api/auth/login` returned `429` from the 8th request onward, confirming the Upstash rate limiter is genuinely engaged. Full end-to-end widget completion could not be visually exercised in this session's sandboxed browser preview, which has no outbound network access to non-allowlisted domains (confirmed independently — `fetch()` to `google.com` also fails from that sandbox) — this is an environment limitation, not an application bug. |
| **BC-14** | Wire `document_access_log`'s `'download'` action into the actual document-download route | G-11 | code | 0.5 day | DONE 2026-09-12 — there was no download route to wire into: `application_documents` files had no fetch path at all (`uploaded_documents` deliberately never stores a raw file). Added `GET /api/documents/[documentId]/download` (commit `78fad69`), which streams the file server-side and logs `'download'` before returning it; refuses with 410 (no storage read attempted) once `file_purged_at` is set. 4 new tests (commit `644d077`) cover the log-on-success, purged-410, not-found-404, and unauthenticated-401 paths. `tsc`/`jest` (709 tests)/`npm run build` all clean. |
| **BC-15** | Document the `retention_hold_at` exception in `docs/DATA_RETENTION_POLICY.md` | G-9 | docs | 0.25 day | DONE 2026-09-12 |
| **BC-16** | Add a soft-delete/archival window (even 7 days, off primary Storage) before `purgeExpiredFiles`/`purgeDeletedAccounts` hard-delete | G-10 | code + infra | 1.5 days | DONE 2026-09-12 — both purge functions hard-deleted Storage objects outright with no recovery path. Migration `20260912180000_soft_delete_archive_window.sql` adds `file_archived_at`/`storage_archive_path`/`archive_hard_deleted_at` to `application_documents`/`fdd_analyses` and a new `archived_account_purges` table (a deleted account's profiles/auth.users row is gone by the time its window elapses, so its pending sweep can't live on that row). `purgeExpiredFiles` and `purgeDeletedAccounts` now `storage.move()` into an `_archive/` prefix instead of removing objects; `file_purged_at`'s existing app-facing meaning ("gone, don't read it") is unchanged and still stamped at the same point. New `sweepArchivedFiles` (`src/lib/retention-cron.ts`) is the only place bytes are actually removed, and only once `ARCHIVE_WINDOW_DAYS` (7) has elapsed — wired into `/api/cron/data-retention`'s `GET` handler after both purge passes. `docs/DATA_RETENTION_POLICY.md` updated with a dedicated section. 3 new tests for `sweepArchivedFiles` (hard-delete past the window, fdd + account-prefix sweep, no-op when nothing's due); existing retention-hold test updated for the archive-move behavior. `tsc`/`jest` (712 tests)/`npm run build` all clean; migration applied to the live database via `supabase db push`. |
| **BC-17** | Consolidate the two `TERMS_VERSION` constants into one shared source | G-14 | code | 0.25 day | DONE 2026-09-12 — `src/middleware.ts`'s terms-acceptance gate and `src/app/api/auth/accept-terms/route.ts` each declared their own local `const TERMS_VERSION = '1.0'`, with no compiler-enforced guarantee they'd stay in sync if one were bumped without the other. Extracted to a new `src/lib/terms-version.ts` exporting the single constant; both sites now import it. No behavior change (same value, same cache-key/DB-column usage). `tsc`/`jest` (712 tests)/`npm run build` all clean. |
| **BC-18** | Write runbooks for the three undrilled failure classes ranked most likely given history: (a) leaked/rotated API key — 13 prior incidents, (b) Stripe outage/webhook backlog, (c) Supabase outage | G-13 | docs | 1 day | DONE 2026-09-12 — new `docs/RUNBOOKS.md`. Runbook 1 (leaked key) tables every key in `.env.example` by blast radius and gives a rotate-in-dashboard → update-in-Vercel → redeploy → verify sequence, with an explicit note that rotating without a redeploy leaves stale keys live on already-running instances. Runbook 2 (Stripe outage) leans on RS-1's `processed_webhook_events` claim semantics to make event-replay via the Stripe Dashboard safe, and points at `payment-reconciliation` (BC-2/BC-3) as the recovery-confirmation check. Runbook 3 (Supabase outage) is explicit that this is a full-outage scenario with no code-level mitigation — do not stand up a parallel DB from a backup mid-incident — and walks through which of the 6 crons are self-healing vs. need a manual re-run afterward, referencing BC-16's 7-day archive window as slack for `data-retention` specifically. Explicitly scopes out what it does NOT cover (Vercel outage, GitHub/Vercel account lockout, multi-provider LLM outage) rather than implying false completeness. |
| **BC-19** | Extend `chaos-drills.mjs` with one drill outside the document pipeline — start with a simulated `payment-reconciliation` mismatch, since BC-2 just wired its alert | G-13 | code | 1 day | DONE 2026-09-12 — added `drill4` to `scripts/chaos-drills.mjs`. Since a real "Stripe shows paid, our records don't" mismatch can't be rehearsed against the live Stripe account (Runbook 2 in `docs/RUNBOOKS.md` rules that out for the same reason), added an env-var-gated fault-injection hook to `/api/cron/payment-reconciliation` (`CHAOS_DRILL_FORCE_PAYMENT_MISMATCH_APP_ID`/`_USER_ID`) that appends one synthetic completed session to whatever the real Stripe account returns — same escape-hatch shape as `CHAOS_DRILL_FAIL_DOC_TYPE`/`CHAOS_DRILL_FAIL_BUILD_DOC_TYPE`, read via `process.env` in the live server process, so the script can't flip it itself and prints a runbook step instead. The drill hits the real `/api/cron/payment-reconciliation` route (not `reconcilePayments()` directly — that's already unit-tested in `payment-reconciliation.test.ts`) and asserts the mismatch is caught, `sendOpsAlert` path is exercised, and `cron_log` records `status='success'` with the mismatch count — proving the ops-alert wiring works end-to-end without the cron itself failing. Two commits, one file each: `1b8e973` (cron route hook), `3b6b42a` (drill4). `tsc`/`jest` (712/712)/`npm run build` all clean. |
...
6. **BC-9 is DONE** (2026-09-12, commit `9f56156`, not yet pushed to `origin/dev` — confirm with Romy first per the standing convention above). Only the `/documents` download-gate consent checkbox was broken; signup and the global `OutcomesConsentBanner` were already correct.
7. **BC-8 is DONE** (2026-09-12, commit `2884c55`, not yet pushed). `/api/documents/route.ts` now extracts text from PDF/DOCX uploads and runs an LLM content-classification pass (same pattern as `detectDocumentType()` / `classifyDocument()` elsewhere in the codebase) as a backstop to the existing client-declared-type check, so a mislabeled identity-document scan gets caught by content instead of sailing through as, say, a "bank statement." Known, accepted scope boundary: scanned (image-only) PDFs have no extractable text and pass through unchecked — same blind spot every other content check in this codebase already has; a real fix there would need OCR, which is out of scope for this task's 1.5-day estimate. Fails open on kill-switch/LLM errors so an outage never blocks a legitimate upload. `tsc`/`jest` (705 tests)/`npm run build` all clean. All Phase 1 `code`-kind tasks not blocked on Romy are now done — remaining Phase 1 items (BC-1, BC-5 through BC-7, BC-12's last step) all need Romy directly, per the items above.
