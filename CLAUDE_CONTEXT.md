...
**⚡ ACTIVE WORKSTREAM (July 14, 2026):** One-Room Redesign of `/onboarding` + `/case-profile` — Sprints K-0…K-5 fully planned and approved, implementation NOT started. **The execution contract is `docs/ONE_ROOM_REDESIGN_PLAN.md` — read it before touching either page or any completion/registry code.** Start order: K-0 (handoff note) → K-1 (field registry + `/api/case/completion`). Hard constraint: `src/app/onboarding/page.tsx` is owned by another agent (uncommitted work in tree) — do not edit it; Sprint K-5 is blocked until their work lands. Approved visuals: https://claude.ai/code/artifact/83cdbf9c-ec05-44dd-97c7-8281203f4161
