...
**Version:** June 30, 2026 — Session 93. Architecture reconciliation (no code changed — option 4 holds). Corrected an overstated "engines are siloed" claim: the flat `answers` table is already a working cross-engine bus (FDD writeback, QMA-* market keys, simulator, voice — all read by generation). Reframed CIC-2 as "upgrade the bus to structured `case_model`/`case_theory`," not "build missing wiring." Recorded pending-decisions register D1–D6 and greenlit CIC-5 (cross-client outcome learning) — all mirrored in `docs/FEATURE_INVENTORY.html` and `BUILD_TRACKER.md`. Session-count estimate given to owner: CIC-2 ~1–2 sessions, full CIC-2→CIC-5 backlog ~4–6 sessions. Full architecture: `docs/sessions/SPRINT_J1_CASE_INTELLIGENCE_CORE.md`. TS clean.
...
- Sprint J-1 (Case Intelligence Core — "the CPU"): 🔶 IN PROGRESS — CIC-0 (grounding: kb_chunks + match_kb RPC + retrieveDoctrine, migration applied/code-complete, KB seeding is the one remaining owner action) and CIC-1 (read-only brain) both code-complete through Session 92. `document-comprehension-engine.ts` (Stage 1 per-doc memo + Stage 2 reconcile → `document_intelligence`), `case-intelligence-core.ts` (`assembleCaseModel` → `case_model`; `generateCaseTheory` → `case_theory`, reasoning as a **five-expert panel**: senior immigration consultant, senior E-2 consular officer, senior immigration attorney, senior franchise development consultant, senior market analyst — synthesized into one Case Theory with persona-tagged creative gap-fill suggestions per dimension). Triggered fire-and-forget from parse-document/answers/simulator-outcome. Surfaced read-only on `/case-profile` (Case Theory section). NOT yet wired into generation/gap-analysis/FDD engines — that's CIC-2, gated behind CIC-1.6 verification (blocked on owner: apply KB seed corpus). **Session 93** reconciled the architecture docs against live code (no code changed) — see Session 93 entry below and in `BUILD_TRACKER.md` for the corrected gap list, the D1–D6 pending-decisions register, and the greenlit-not-built CIC-5 (cross-client outcome learning) backlog. Full detail: `docs/sessions/SPRINT_J1_CASE_INTELLIGENCE_CORE.md`.
