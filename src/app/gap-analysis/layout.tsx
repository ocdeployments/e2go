...
async function buildSteps(): Promise<SectionStep[]> {
  const staticSteps: SectionStep[] = [
    ...
    { id: 'generate',    label: 'Generate',      href: '/apply',            done: false },
  ...
  try {
    ...
    const [quizRes, lifecycleRes, simRes, docRes] = await Promise.all([
      supabase.from('quiz_sessions').select('completed_at').eq('user_id', user.id).order('completed_at', { ascending: false }).limit(1).maybeSingle(),
      supabase.from('application_lifecycle').select('module1_completed_at, module2_completed_at, module3_completed_at, module5_completed_at').eq('user_id', user.id).maybeSingle(),
      supabase.from('simulator_sessions').select('id', { count: 'exact', head: true }).eq('user_id', user.id).not('completed_at', 'is', null),
      primaryAppId
        ? supabase.from('generated_documents').select('id', { count: 'exact', head: true }).eq('application_id', primaryAppId)
    ...
    return [
      ...
      { id: 'gap',         label: 'Gap Analysis',  href: '/gap-analysis',     done: false },
      { id: 'generate',    label: 'Generate',      href: '/apply',            done: docCount >= 15, sublabel: docCount > 0 ? `${docCount} / 15 ready` : undefined },
