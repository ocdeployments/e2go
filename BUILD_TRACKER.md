...
**Last Updated:** September 6, 2026 — Session 133: Ran an adversarial security pass on the checkout/payment flow at Romy's request ("act as a hacker... see if you can bypass paying"). Found and fixed a real IDOR: `create-checkout` trusted a client-supplied `applicationId` with no ownership check, and that unchecked ID flowed into Stripe metadata where the webhook, `verify-payment`, and the loyalty-pricing check all trusted it again downstream — an attacker could mark someone else's application `paid` or fraudulently qualify for loyalty pricing. Fixed at the root and at all three downstream consumers (defense in depth). Also closed a smaller gap: `/api/promo/validate` had no rate limit — added one. A broader background sweep of ~44 other API routes for the same IDOR pattern then returned 7 more findings (2 HIGH, 4 MEDIUM, 2 LOW) — all fixed with the same ownership-check pattern; see item 3 in the session log below for the full list. `npx tsc --noEmit` and `npm run build` both clean after all fixes. Assessed document storage (request: "store documents so we don't start from scratch") — this **already substantially exists** (`generated_documents.content_json`/`content_text`, rebuilt to a ZIP on download without re-running the LLM); real gap is a browsing/reproduce UI, not storage. Drafted a self-learning design (request: platform should learn from what it generates) using existing `document_change_log` + `document_generation_log` tables as the raw signal — proposed as a sprint, not yet built. Answered Romy's leaked-code/single-use/MFA question (personal codes are matched to the caller's own server-derived email, one redemption per account is enforced by a DB-level partial unique index, MFA isn't needed for this specific threat) and, on her go-ahead, resolved the promo-code-minting-mechanism question by **building a full admin page** at `/admin/promo-codes` (create/list/activate/deactivate — no CLI or engineer needed). Also discovered **zero production accounts had admin role, including Romy's own** — fixed with her explicit approval by granting her account admin permanently. Live-testing the admin page then surfaced a bigger issue: **the `promo_codes`/`promo_redemptions` tables were never actually created in production** — the migration file existed but had never been applied, confirmed via two independent read-only checks (PostgREST returned the tables missing from its full 75-table schema list, not just a stale-cache symptom). Handed to Romy to run herself in the Supabase SQL Editor, per the standing rule against Claude running SQL directly against production — she ran it, and the full create → deactivate → activate → delete flow was then live-verified end-to-end with her real admin session. **The promo-code admin page is now fully live and working in production.** See items 8–9 in the session log below. Romy reviewed the findings and said "commit them now" — all of it is committed as of this update, one file per commit, 16 commits total: the 3 promo-feature-core commits (tables, validation/redemption/rate-limiting, admin page), the full 11-file IDOR sweep (item 1's `create-checkout`/`webhook`/`verify-payment`/`entitlements.ts` fix plus all 7 item-3 findings), and 2 bonus fixes surfaced while reviewing diffs for scope during the commit pass, neither previously written up here: `src/app/api/checkout/initiate/route.ts` still referenced retired tier IDs (`complete`, `complete_partnership`, `fdd_intelligence`) after `pricing-tier.ts` had already moved to the new tier model in an earlier session — meaning checkout via this route for Foundation/Investor Ready/Visa Ready was silently broken until this fix, which also wires in the same promo-code reserve/apply/release flow as `create-checkout`; and `src/app/pricing/PricingClient.tsx` had no way for a customer to actually enter a promo code — the admin page and both checkout routes could create/redeem codes, but `/pricing` itself, the only real purchase entry point, had no input for one, so wired in the existing `PromoCodeInput` component. See the commit list at the end of this session's log for the full mapping. **Session 132's pricing pivot (Texas/US-only/USD-only, remove Canadian pricing) is still the standing next priority and was not touched this session** — see that entry below.
