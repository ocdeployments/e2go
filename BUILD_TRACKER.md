...
**Last Updated:** September 14, 2026 — Session 153: session-start audit + results-email review. No code committed. See **"Session 153 — Session-start audit: WHAT IS CURRENTLY LACKING"** below for the open items found: `Security Gates` red on `main` (TruffleHog config bug, `.github/workflows/security.yml:86-98`), `origin/main` 17 commits behind `origin/dev`, an uncommitted FTC testimonials-removal fix sitting in the working tree, a dead `markTokenUsed()` that makes the results email's "Your link opens once" claim factually false, and a stale ROUTE MAP in `CLAUDE_CONTEXT.md` (claims 60 routes; actual 110 pages + 124 API).

**Previously:** September 12, 2026 — Session 152 (cont.): **closed BC-19 (Gap G-13) — the last item in Sprint BC's active queue.** Added `drill4` to `scripts/chaos-drills.mjs`: a real "Stripe shows paid, our records don't" mismatch can't be safely rehearsed against the live Stripe account (the same reason Runbook 2 in `docs/RUNBOOKS.md` rules that action out for a human responder), so instead of drilling against real Stripe, added an env-var-gated fault-injection hook to `/api/cron/payment-reconciliation` (`CHAOS_DRILL_FORCE_PAYMENT_MISMATCH_APP_ID`/`_USER_ID`) that appends one synthetic completed session — scoped to the chaos persona's own applicationId/userId — to whatever the real Stripe account returns. This follows the exact escape-hatch shape DR-21 already established with `CHAOS_DRILL_FAIL_DOC_TYPE`/`CHAOS_DRILL_FAIL_BUILD_DOC_TYPE`: read via `process.env` inside the live Next.js server process, so the script can't flip it itself and prints a manual runbook step (set the two env vars, restart `npm run dev`) before the drill can observe a real mismatch. `drill4` hits the real `/api/cron/payment-reconciliation` HTTP route rather than calling `reconcilePayments()` directly (that function already has full unit coverage in `payment-reconciliation.test.ts` with fake Stripe/Supabase clients) — this is deliberately an operational drill proving the whole chain (route → `reconcilePayments` → `sendOpsAlert` → `cron_log`) works end-to-end against the live dev server, which the unit tests don't exercise. Asserts the cron returns `mismatches >= 1` and that `cron_log` records `status='success'` with the mismatch count in its metadata — confirming the mismatch is caught and alerted on without the cron itself failing. Three commits, one file each: `1b8e973` (cron route fault-injection hook), `3b6b42a` (drill4), `a646645` (sprint doc marked BC-19 `DONE`). `tsc`/`jest` (712/712)/`npm run build` all clean. **This closes the full BC-13→BC-19 sequence** — Sprint BC's active queue is now empty; everything remaining in `docs/SPRINT_BC_BUSINESS_CONTINUITY.md` (BC-1, BC-5/6/7, BC-12's final step) is explicitly blocked on Romy, not something the next agent should pick up unilaterally. **Not pushed to `origin/dev`** — confirm with Romy first; the unpushed stack now spans BC-13 through BC-19 plus everything before. **Next agent:** no open BC item — check with Romy for the next sprint/priority before starting new work.

**Prior — Session 152 (cont.): closed BC-18 (Gap G-13) — wrote runbooks for the three undrilled failure classes.** New `docs/RUNBOOKS.md` covers the three highest-likelihood failure domains the sprint doc flagged as never rehearsed outside document generation: leaked/rotated API keys (13 prior real incidents — the project's actual most-common failure), a Stripe outage/webhook backlog, and a Supabase outage. Each runbook is grounded in this codebase's real mechanisms rather than generic advice: Runbook 1 tables every key in `.env.example` by blast radius and calls out that rotating a key without a redeploy leaves the old key live on already-running serverless instances; Runbook 2 leans on RS-1's `processed_webhook_events` claim/reclaim semantics to make Stripe Dashboard event-replay safe, and uses `payment-reconciliation` (BC-2/BC-3) as the recovery-confirmation check; Runbook 3 is explicit that a Supabase outage has no code-level mitigation — do not stand up a parallel DB from a backup mid-incident — and walks through which of the 6 Vercel crons are self-healing vs. need a manual re-run, citing BC-16's new 7-day archive window as slack for `data-retention` specifically. Explicitly scopes out what's NOT covered (Vercel outage, GitHub/Vercel account lockout, multi-provider LLM outage) instead of implying false completeness. Docs-only task — no code changed, so verified by review rather than the tsc/jest/build gates (all still clean, unaffected). Two commits: `57ca8c4` (runbooks doc), `8625f20` (sprint doc marked BC-18 `DONE`). **Not pushed to `origin/dev`** — confirm with Romy first. **Next agent:** BC-19 (extend `scripts/chaos-drills.mjs` with a payment-reconciliation drill, G-13, code, 1 day) is the last item in this sprint's active queue — `scripts/chaos-drills-lib.mjs` and `scripts/fixtures/` are pre-existing untracked files flagged as relevant; inspect them before writing the new drill. — Session 152 (cont.): **closed BC-17 (Gap G-14) — consolidated the two independently-declared `TERMS_VERSION` constants into one shared source.** `src/middleware.ts`'s terms-acceptance gate and `src/app/api/auth/accept-terms/route.ts` each hardcoded their own local `TERMS_VERSION = '1.0'`, with nothing enforcing they'd stay in sync if the terms were ever revised and only one site got bumped — the gate would then check against a version the acceptance write never recorded. Extracted to new `src/lib/terms-version.ts` (single exported constant); both sites import it, zero behavior change (same value, same cache-key and DB-column usage). Three commits, one file each: `2added1` (new shared constant), `6b8271c` (middleware.ts), `91b9888` (accept-terms route.ts), plus `64426d3` (sprint doc marked BC-17 `DONE`). `tsc`/`jest` (712/712)/`npm run build` all clean. **Not pushed to `origin/dev`** — confirm with Romy first. **Next agent:** BC-18 (runbooks for leaked-API-key/Stripe-outage/Supabase-outage, G-13, 1 day) is next, followed by BC-19 (payment-reconciliation chaos drill). — Session 152 (cont.): **closed BC-16 (Gap G-10) — a 7-day Storage recovery window before hard-delete, replacing outright irreversible purges.** `purgeExpiredFiles` and `purgeDeletedAccounts` used to call `storage.remove()` directly, so a bug in the purge logic or a wrongly-set `file_purged_at` was unrecoverable the instant the cron ran. Preserved the existing app-facing contract exactly — `file_purged_at` still means "gone" and is still stamped at the same point — but underneath, objects now move via `storage.move()` to an `_archive/` prefix in the same bucket instead of being deleted outright. New migration `20260912180000_soft_delete_archive_window.sql` (applied live via `supabase db push --workdir /Users/owner/E2-go`, confirmed "Finished supabase db push") adds `file_archived_at`/`storage_archive_path`/`archive_hard_deleted_at` to `application_documents` and `fdd_analyses`, plus a new `archived_account_purges` table (RLS, service-role only) tracking deleted-account Storage sweeps since the owning `profiles`/`auth.users` row is gone by then. New `sweepArchivedFiles()` (`src/lib/retention-cron.ts`) is the only place bytes are actually removed — hard-deletes anything in `_archive/` older than `ARCHIVE_WINDOW_DAYS` (7). `data-retention/route.ts`'s account-purge step now archives each object individually and logs one `archived_account_purges` row instead of removing outright; the `GET` handler runs `sweepArchivedFiles` and reports it alongside the other jobs. `docs/DATA_RETENTION_POLICY.md` updated with a new "Storage archive window" section and schedule-table notes. Seven commits, one file each: `443a916` (migration), `34031c6` (retention-cron.ts archive-move + sweep), `b2129a5` (data-retention route archive-on-delete), `b8a63e4` (updated retention-hold test for move-not-remove), `6190b47` (3 new tests for `sweepArchivedFiles`), `8f04bef` (policy doc), `fc612e8` (sprint doc marked BC-16 `DONE`). `npx tsc --noEmit`, full `npx jest` (712/712), and `npm run build` all clean. **Not pushed to `origin/dev`** — confirm with Romy first. **Next agent:** BC-17 (consolidate the two `TERMS_VERSION` constants, G-14, 0.25 day) is next per the sprint doc's sequencing, followed by BC-18 (runbooks for leaked-API-key/Stripe-outage/Supabase-outage) and BC-19 (extend `scripts/chaos-drills.mjs` with a payment-reconciliation drill). — Session 152 (cont.): **closed BC-14 and BC-15.** BC-14 (G-11): `document_access_log`'s `'download'` action was a defined type nobody ever wrote — there was in fact no download route at all for `application_documents` files (the only document type that stores a raw file at all; `uploaded_documents` deliberately never does, per CLAUDE.md's identity-document policy). Added `GET /api/documents/[documentId]/download` (`78fad69`): streams the file server-side rather than handing back a client-usable signed URL, so every fetch is logged before the bytes go out; returns 410 without touching storage once `file_purged_at` is set (the retention cron already purged it). 4 new tests (`644d077`) cover log-on-success, purged-410, not-found-404, unauthenticated-401 — mocked the same way DR-19's `application-type.test.ts` mocks `createSupabaseServerClient`, since this route (like most here) has no DI seam for a real integration test. BC-15 (G-9): `docs/DATA_RETENTION_POLICY.md` never mentioned `retention_hold_at` — a real, code-enforced, **permanent** exception in `purgeExpiredFiles` (set once via the confirm-to-keep email link, never cleared, never re-checked) that lets a user opt an application out of the 30/90-day file-purge schedule indefinitely. Added a dedicated section explaining the trigger, one-way nature, and scope (`54ea4df`). `docs/SPRINT_BC_BUSINESS_CONTINUITY.md` updated for both (`76dc838`). `tsc`/`jest` (709/709)/`build` all clean. Six more commits stacked on `dev`, still unpushed pending Romy's confirmation. — Session 152 (cont.): **closed BC-13 — proxied login through `/api/auth/login`, closing the second half of G-12, and found + fixed a pre-existing CSP bug that silently broke Turnstile everywhere it's configured.** Picked up per Romy's own "start on BC-13" instruction. Prior work (a previous session) had already built `/api/auth/signup` and `/api/auth/login` as server-side proxies wrapping `checkRateLimit()` (Upstash, `'auth'` profile, 8/10min IP-keyed) and `verifyTurnstile()` (fail-closed on a genuine network/timeout error, skip-only-if-unconfigured) around the actual Supabase Auth calls, and wired signup's page to call it — but [login/page.tsx](src/app/login/page.tsx) still called `supabase.auth.signInWithPassword()` directly from the browser, bypassing both controls entirely for half of G-12's stated scope. Rewired `handleLogin` to `fetch('/api/auth/login', ...)` instead, sourcing the existing remember-me/quiz-session-linking/draft-import/tracking/smart-routing logic from `supabase.auth.getSession()` afterward (the proxy's `Set-Cookie` response lands on the fetch, so the browser's `@supabase/ssr` client picks up the new session transparently) — zero behavior change to any of that downstream logic. Added the same Turnstile widget/guard/disabled-button pattern signup already had. Also deleted `src/app/api/auth/verify-captcha/route.ts` (zero remaining callers, made redundant by the two routes now doing their own inline verification) and documented both Turnstile env vars in `.env.example`. **Live-testing this in the browser surfaced a real, independent production bug**, also in scope of G-12 since it affects both login and signup: `next.config.mjs`'s CSP `script-src` was missing `https://challenges.cloudflare.com` and there was no `frame-src` at all — so Turnstile's `api.js` never loaded, and its container div (`<div id="cf-turnstile" style="width:300px;height:65px">`) sat empty but still intercepted pointer events, silently swallowing every click on the Sign In button with zero error shown to the user. This means **login has likely been completely broken** (not just "CAPTCHA missing" — totally unusable, submit does nothing) in any environment with `TURNSTILE_SITE_KEY` configured, which is true of this project's own `.env.local` right now — and since signup uses the identical widget pattern, it was equally broken, predating this session entirely. Fixed by adding `challenges.cloudflare.com` to `script-src` and `connect-src`, and adding `frame-src https://challenges.cloudflare.com` for the challenge iframe. Verified live against the running dev server: `fetch('/login')`'s response headers confirmed the corrected CSP is actually served; the submit button's `disabled` state confirmed `true` while no CAPTCHA token is present (the client guard fires correctly); `curl` against both `/api/auth/login` and `/api/auth/signup` confirmed 400 "Missing CAPTCHA token" with none supplied and 400 "CAPTCHA verification failed" with a bogus one — proving the fail-closed path is real, not just code that looks right; 9 rapid `curl` requests to `/api/auth/login` returned `429` from the 8th request onward, proving the Upstash rate limiter is genuinely engaged on the proxied route. **Could not visually verify the Turnstile widget rendering or complete a full login round-trip through the browser preview** — this session's sandboxed Browser pane has no outbound network access to non-allowlisted external domains at all (confirmed independently: a bare `fetch('https://www.google.com')` from that same sandbox also fails, while `https://api.anthropic.com` succeeds), which is an environment limitation of the tool itself, unrelated to the app's own CSP or code. Four commits, one file each: `178d922` (delete unused verify-captcha route), `2e23463` (login proxy rewrite + Turnstile widget), `aae675b` (.env.example docs), `5517cdd` (CSP fix). `npx tsc --noEmit`, full `npx jest` (705/705), and `npm run build` all clean at every commit. `docs/SPRINT_BC_BUSINESS_CONTINUITY.md` updated: BC-13 marked `DONE 2026-09-12` (`cd86ced`). **Not pushed to `origin/dev`** — confirm with Romy first, and this stack is now larger than prior sessions' — worth a single confirm-and-push conversation rather than piecemeal. **Next agent:** per Romy's own follow-up instruction mid-session, continue sequentially through BC-14 (wire `document_access_log`'s `'download'` action into the real download route), BC-15 (document the `retention_hold_at` exception in the retention policy), BC-16 (soft-delete/archival window before hard-delete purges), BC-17 (consolidate the two `TERMS_VERSION` constants), BC-18 (runbooks for leaked-API-key/Stripe-outage/Supabase-outage scenarios), BC-19 (extend `chaos-drills.mjs` with a payment-reconciliation drill) — all Phase 2 `code`/`docs` tasks not blocked on Romy. BC-1/BC-5/BC-6/BC-7/BC-12's final GitHub-secrets step remain blocked on Romy directly, unchanged. — Session 152 (cont.): **closed BC-8 — content-based backstop for identity-document rejection on `/api/documents` (G-4).** Picked up as the next Phase 1 `code`-kind task not blocked on Romy, per the prior entry's own "Next agent" note. The route rejected passport/birth-certificate/etc. uploads by looking only at the client-declared `type_${key}` form field — a mislabeled or relabeled identity-document scan (e.g. filed as "bank statement") would sail through untouched and get stored as a raw file, directly contradicting the "identity documents are never stored as files" policy both `docs/DATA_RETENTION_POLICY.md` and the project `CLAUDE.md` assert as a hard rule. Investigated the existing content-classification infrastructure before building anything new: `classifyDocument()` (`document-extraction-engine.ts`) runs post-upload for extraction routing and doesn't cover identity types at all; `detectDocumentType()` (`/api/apply/parse-document`) does cover `passport`/`birth_certificate`/`marriage_certificate` but only runs on the identity-intake path, which already legitimately discards the file — neither was reusable as-is for a pre-storage gate on the business-document upload path. Fixed in [`route.ts`](src/app/api/documents/route.ts) (commit `2884c55`) by adding a same-pattern LLM content check: for PDF/DOCX uploads, extract text via the existing `extractTextFromBuffer()` and run one classification call (`task: 'extract'`, matching `detectDocumentType`'s own model tier) asking whether the content looks like a government identity document, independent of what the client declared — reject with the existing error message if so. Deliberate scope boundary, called out in both the code comment and the sprint doc: a scanned/image-only PDF has no extractable text to classify and passes through unchecked — the same blind spot every other content check in this codebase already has; closing that would need OCR, out of scope for the 1.5-day estimate. Fails open (upload proceeds, error captured to Sentry) on a kill-switch-enabled or LLM-failure case, so an infra hiccup never blocks a legitimate financial-document upload — this is a bolted-on safety backstop, not the route's primary purpose, so failing closed here would be the wrong tradeoff. No component-test harness exists in this repo for API routes with real Supabase/LLM calls, so this was verified via the standard `tsc --noEmit` (clean) / full `npx jest` (705/705) / `npm run build` (clean) gates rather than a live walkthrough — a pure server-side content-inspection change with no new UI surface. Two commits, one file each: `2884c55` (code), `3ca54f1` (`docs/SPRINT_BC_BUSINESS_CONTINUITY.md` marked BC-8 `DONE 2026-09-12`). **Not pushed to `origin/dev`** — confirm with Romy first. **Next agent:** all Phase 1 `code`-kind tasks not blocked on Romy are now done (BC-8, BC-9, BC-10, BC-11 all closed); everything remaining in the sprint doc (BC-1 disclose-vs-remove decision, BC-5 through BC-7 GitHub repo settings, BC-12's final GitHub Actions secrets step) needs Romy directly — surface those rather than picking up further BC work unilaterally. There is also an unpushed, untracked commit (`6465afb`, "Fix db-backup workflow to install matching pg_dump version") sitting on `dev` from before this session's BC-9/BC-8 work that was never called out in a tracker entry — worth flagging to Romy alongside the rest before any push. — Session 152 (cont.): **closed BC-9 — fixed the third, broken D6 outcomes-consent touchpoint on `/documents`.** Picked up from the sprint doc's own "Next agent — start here" list as the next `code`-kind task not blocked on Romy. Investigation found the task was scoped larger than reality: the persistence layer (`profiles.outcomes_consent`/`outcomes_consent_at`, `/api/profile/outcomes-consent`) already existed and was already correctly wired at signup and the global-nav `OutcomesConsentBanner` — no migration needed. Only the third touchpoint, the pre-download acknowledgment gate on `/documents`, was broken: it bundled an `outcomes_consent` checkbox into the mandatory legal-disclaimer acknowledgments object without ever calling the persistence API, so it neither gated correctly nor recorded anything real. Fixed in [`page.tsx`](src/app/documents/%5BapplicationId%5D/page.tsx) (commit `9f56156`) — split it into its own state wired to the same fetch-on-mount/persist-on-toggle pattern as the banner, moved out of the 5-item mandatory checklist into its own visually-distinct optional control (divider, `text-white/50`) so it can never block or unblock the download button. No component-test harness exists in this repo (`jest.config.js` is `testEnvironment: "node"`, no jsdom/RTL) so this UI-only change was verified live rather than via a named unit test, consistent with the project's Playwright-for-UI convention: logged into a real fixture account with generated documents (`test-uk@example.com`) via the magic-link-mint + `setSession()` technique, temporarily forced `packageReady = true` client-side (tagged `// TEMP-BC9-VERIFY`, fully reverted before commit, confirmed via `git diff` + `grep`) to render the otherwise-unreachable gate UI, and confirmed all three required behaviors: the 5 mandatory checkboxes still gate the download button unchanged, the new optional checkbox never affects `canDownload`, and toggling it round-trips through `/api/profile/outcomes-consent` (200 in server logs, persisted value survives reload). `tsc --noEmit`, full `npx jest` (705/705), and `npm run build` all clean. Two commits, one file each: `9f56156` (code), `f8bae0a` (`docs/SPRINT_BC_BUSINESS_CONTINUITY.md` marked BC-9 `DONE 2026-09-12`). **Not pushed to `origin/dev`** — confirm with Romy first. **Next agent:** BC-8 (identity-document rejection by file content, not client-declared type) is the other Phase 1 `code`-kind task not blocked on Romy and is next up per the sprint doc — not yet started. — Session 152 (cont.): **confirmed everything is caught up across `dev`/`origin/dev`/`origin/main`/local `main`, and gave Romy a live look at the DR-20 pilot cell.** `git fetch` showed local `dev` fully in sync with `origin/dev` at `dab893b` (nothing unpushed) and `origin/main` already 0 commits behind `origin/dev` — the two BC-12/DR-20 doc-update commits from earlier this session (`2dcd263`, `d3e3c91`) had already gone out via **[PR #4](https://github.com/ocdeployments/e2go/pull/4)** (`dev` → `main`, merged as `025a9aa`). The only stale thing was the **local** `main` branch pointer, sitting 47 commits behind `origin/main` since no one had pulled it recently — fast-forwarded `main` from `8045b06` to `025a9aa` (no working-tree changes, session stayed on `dev` throughout). Separately, logged Romy into the one seeded DR-20 pilot cell (`dr20-solo-independent-canada-funded@e2go-test.internal`, "Jordan Whitfield") via the standard magic-link-mint + `setSession()` browser-injection technique so she could inspect `/case-profile` herself rather than take a status report on faith — required restarting the dev server first (a concurrent `npm run build` from the pre-push hook had corrupted its `.next` chunk manifest) and navigating via `/quiz` first (a public route that eagerly initializes the app's browser Supabase singleton) before the gated `/case-profile` would accept the injected session. — Session 152 (cont.): **closed BC-12 — independent Supabase database backup path, built per Romy's step-by-step walkthrough choice (Option B).** BC-4 had already confirmed the Supabase project is on the free tier (no PITR, no automated backups), making this the app's only real recovery path if the Supabase project is lost or corrupted. Provisioned a Cloudflare R2 bucket (`e2go-db-backups`) + a scoped API token (Object Read & Write, bucket-specific) via a live Cloudflare-dashboard walkthrough; generated a Supabase `DATABASE_URL` pooler credential; added all 5 credentials (`DATABASE_URL`, `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET_NAME`) to Vercel (the DB password was entered by Romy directly via the dashboard, never passed through a CLI arg or chat). Chose **GitHub Actions over a Vercel cron route** (via AskUserQuestion) because Vercel's serverless runtime has no `pg_dump` binary; GitHub's `ubuntu-latest` runners can `apt-get install postgresql-client`. Retention: **7 daily + 4 weekly**, rotated in R2. Built `.github/workflows/db-backup.yml` (commit `2d8c32d`) — dumps daily, promotes Sunday's dump to weekly, rotates old backups via `aws s3api`/`aws s3 rm`, logs every run to `cron_log`. **Still open**: the same 5 credentials need to be added separately to GitHub Actions repository secrets (Vercel env vars don't carry over) before the workflow can actually run — not yet done. `docs/SPRINT_BC_BUSINESS_CONTINUITY.md` updated: BC-12 marked `DONE 2026-09-12`. **Not pushed to `origin/dev`** — confirm with Romy first. **Next agent:** walk Romy through adding the 5 GitHub Actions secrets (Settings → Secrets and variables → Actions, in `ocdeployments/e2go`), then trigger a manual `workflow_dispatch` run to confirm a backup lands in R2 and a `cron_log` row is written; BC-1 and BC-5/6/7 still need Romy directly. — Session 152 (cont.): **shipped BC-10 — fixed `health-watchdog`'s consecutive-failure cron watchlist (G-7).** `health-watchdog`'s `CRON_JOBS` array named a nonexistent `'email-scheduler'` job (no such cron exists in `vercel.json`) and never watched `generation-resume`/`payment-reconciliation`/`data-retention` at all. Before changing the list, verified each candidate route actually writes to `cron_log` (a watchlist entry for a job that never logs is a silent no-op) — `generation-resume` and `payment-reconciliation` already do; `data-retention` did not, so `src/app/api/cron/data-retention/route.ts` now inserts a `running` row before the sweep and updates it to `success`/`failed` after, matching `payment-reconciliation`'s pattern. **New finding surfaced, not fixed this session:** `rebuild-profiles` was already on the old watchlist but its route also never writes to `cron_log` — that check has likely never fired either; dropped from the list rather than left in as false coverage, flagged in the sprint doc as a small follow-up. `CRON_JOBS` is now `['generation-resume', 'payment-reconciliation', 'data-retention']`. Three commits, one file each: `549d7ee` (data-retention cron_log instrumentation), `b7060ee` (health-watchdog watchlist fix), `ae2d664` (sprint doc). `tsc --noEmit`, full `npx jest` (705 tests), and `npm run build` all clean at each commit (one lint-error-as-CI-error was caught and fixed mid-task: the new `cron_log` insert needed its `error` bound per this repo's `require-supabase-error-check` rule). `docs/SPRINT_BC_BUSINESS_CONTINUITY.md` updated: BC-10 marked `DONE 2026-09-12`. **Not pushed to `origin/dev`** — confirm with Romy first. Phase 0's only remaining items are still BC-1 and BC-4 (both need Romy directly). **Next agent:** BC-11 (ops alert destination + fail-loudly on missing `RESEND_API_KEY`, G-8) is next in Phase 1 — its fail-loudly half is `code`, its alert-destination half is `decision` (whose inbox), keep them separate; do not pick the destination unilaterally. — Session 152 (cont.): **committed the audit/sprint docs, then shipped BC-2 and BC-3 — active alerts on the two silent-failure paths the audit flagged (G-7).** `src/lib/payment-reconciliation.ts` now calls `sendOpsAlert()` (reusing the existing helper, not a new channel) on every Stripe/records mismatch instead of only `captureApiError`. `src/lib/generation-engine.ts`'s top-level `fail()` handler — previously `console.error` only, zero Sentry, zero alert, on the pipeline the whole product depends on — now calls `Sentry.captureException` and `sendOpsAlert()` with applicationId/userId/jobId/step context. Four commits, one file each: `6257091` (gap register), `886c76f` (sprint plan), `6176716` (tracker), `9b681d1` (BC-2), `eeb9a58` (BC-3) — `tsc --noEmit`, full `npx jest` (705 tests), and `npm run build` all clean at every commit. **Not pushed to `origin/dev`** — confirm with Romy first. `docs/SPRINT_BC_BUSINESS_CONTINUITY.md` updated: BC-2/BC-3 marked `DONE 2026-09-12`. Phase 0's only remaining items are BC-1 (sub-processor disclose-vs-remove decision) and BC-4 (Romy's 15-minute Supabase dashboard check) — both need Romy directly, not code. **Next agent:** do not start Phase 1 (BC-5 onward) until BC-4's answer is in, since it resizes BC-12 and possibly the whole phase's effort estimate; surface BC-1 and BC-4 to Romy first. — Session 152: **ran a five-hat business-continuity/disaster-recovery audit (SRE, Database Optimizer, Security Architect, Compliance Auditor, Incident Responder), scored the audit itself at Romy's request, then wrote and self-scored a mitigation sprint — planning and docs only, no code shipped.** Romy's ask, staged explicitly over several turns: (1) a thorough multi-perspective gap analysis, (2) score that report and find its own weaknesses before acting on it, (3) only then build a mitigation plan, scored the same way. Five parallel read-only subagents produced 16 findings (1 CRITICAL, 5 HIGH, 7 MEDIUM, 3 LOW), then this session verified several "unverifiable from repo alone" items directly rather than leaving them as guesses: `vercel env ls production` confirmed **no `DATABASE_URL`/`POSTGRES_URL`/`SUPABASE_DB_URL` exists anywhere** (the app has never had a path to run its own Postgres backup — recovery depends entirely on Supabase's own managed backup, whose plan tier/PITR window is dashboard-only and still unconfirmed); `gh api` confirmed the repo is **PUBLIC**, `dependabot_security_updates` is **disabled**, `secret_scanning`+`push_protection` are **enabled**, and both `main` and `dev` have **zero branch protection and zero rulesets**. The CRITICAL finding: `llm-client.ts`'s extraction/FDD fallback chains route document text to Zhipu AI (`z-ai/glm-5.2`) and Xiaomi (`mimo-v2.5`/`-pro`) and direct Anthropic, none of which are disclosed anywhere the Privacy Policy or `DATA_RETENTION_POLICY.md` names Google/Gemini as the only sub-processor — a live compliance gap, not a theoretical one. Full findings: `docs/BUSINESS_CONTINUITY_GAPS.md` (G-1 through G-16). Mitigation plan: `docs/SPRINT_BC_BUSINESS_CONTINUITY.md` (BC-1 through BC-22, phased Phase 0 "stop the bleeding this week" → Phase 3 "long-tail"), self-scored in the same doc (coverage 9/10, sequencing 8/10, effort-estimate confidence 6/10 — flagged as rough-order-of-magnitude only, decision-vs-code discipline 9/10, independent verifiability 7/10). **Nothing built or committed this session** — two new doc files only (`docs/BUSINESS_CONTINUITY_GAPS.md`, `docs/SPRINT_BC_BUSINESS_CONTINUITY.md`) plus this entry; per this project's own "no task is Done on reasoning" convention, every BC-# task starts at `TODO`. **Next agent:** Phase 0 first, in isolation — surface BC-1 (disclose vs. remove the undisclosed LLM sub-processors) and BC-4 (Romy checks the Supabase dashboard for real PITR/backup status) as decisions/manual checks before writing any code; BC-2/BC-3 (wire an active alert onto `payment-reconciliation.ts` mismatches and `generation-engine.ts`'s top-level failure handler, both currently Sentry-passive or silent) can start immediately in parallel since they don't depend on either decision. Hold every Phase 1 effort estimate as provisional until BC-4's answer is in — it could shrink Phase 1 (PITR already adequate) or make BC-12 the sprint's largest single task (no independent backup path exists and one has to be built from scratch). — Session 151: **audited synthetic test-data completeness for DR-20, then designed and got approval for a real-pipeline 36-cell delivery matrix — planning only, no code yet.** Romy's ask, in order: (a) audit whether the existing seeded test accounts (Canada/France/UK/Partnership) are complete enough for a full DR-20 run, (b) produce a plan before building anything, (c) only then implement and walk the app end-to-end in the browser. The audit found the fixtures are not complete: only 1 of 4 accounts has a populated Module 3 answer set, none populate `case_theory`/`document_intelligence`/`case_brief_json`/`applicant_voice_profile`/`followup_responses`, and none cover independent business type, partial funding, or the three nationalities DR-20 needs. Drafted a plan, then ran it through expert self-review at Romy's request (twice) before she'd approve it — the review caught two real problems: the first draft hand-injected `case_theory`/`case_brief_json`/`document_intelligence` as fixture data instead of deriving them from the real pipeline (the one thing DR-20 exists to test), and no separate dev/staging Supabase project exists (`vercel env pull` confirms Development and Production share one project), which meant a synthetic partnership fixture's fabricated `payments` row could page Romy via `health-watchdog`, inflate `admin/revenue`, or trip `payment-reconciliation`. Both fixed in the revised design: everything with a real producer (`case_theory`, `document_intelligence`, `case_brief_json`, the generated package) is derived by driving the app's actual routes over authenticated HTTP (magic-link session, same technique as `run-persona-generation.mjs`) instead of hand-authored; a new `payments.is_test_fixture` column plus one shared exclusion helper wired into the three at-risk readers closes the isolation gap. Romy's third round of feedback was a direct question rather than another rescore — "what will we have in data once this is approved, what can we actually test" — answered by adding a data-provenance table and an explicit "what this run does and doesn't test" section to the plan; that answer is what got it approved. **Design is final; nothing has been built** — no migration, no `seed-delivery-matrix.mjs`/`delivery-matrix.mjs`/`teardown-delivery-matrix.mjs`, no seeding, no generation run. Full plan: `.claude/plans/wobbly-bubbling-plum.md`. Updated `docs/SPRINT_DR_DELIVERY_RELIABILITY.md`'s DR-20 section (real-pipeline design, the `is_test_fixture` mitigation, Japan/Canada/South Korea as the three nationalities, revised effort estimate ~4 eng-days/$300–500 LLM given the ~8,000-call seeding phase) and the Phase 8 effort-and-sequencing row — DR-20's task-table status stays `TODO`, per this doc's own "no task is Done on reasoning" rule. Nothing committed yet (docs-only changes; confirm with Romy before committing). **Next agent:** implement the plan's Build steps 0–5 in order (migration + shared helper → `seed-delivery-matrix.mjs` → `delivery-matrix.mjs` → dated results table → UI walkthrough via the magic-link login technique → `teardown-delivery-matrix.mjs`); the plan file has full detail including the exact `/api/answers` one-key-per-call constraint and the 30-second case-intelligence lock behavior that affects seeding completion detection. — Session 150: **verified both remaining Phase 1 migrations live, closed out RS-1, and shipped Sprint RS Phase 1 (RS-1 through RS-4) plus RS-5/RS-7/RS-10 to production.** Romy pasted both pending migrations into the Supabase Dashboard SQL Editor by hand (`20260910160000_webhook_dedup_status.sql` for RS-1, `20260910130000_generation_resume_log.sql` for Sprint DR's checkpointed-resume table) and asked for schema verification. Confirmed both live via the live-schema-is-truth rule: a clean `scripts/audit-schema-drift.py --refresh` run, plus direct PostgREST queries (`processed_webhook_events?select=status&limit=1` → `[{"status":"completed"}]`; `generation_resume_log?select=id&limit=1` → `[]`, no `42P01`). `npx supabase migration repair` still fails in this sandbox (`LegacyPlatformAuthRequiredError` — no cached access token/DB password, same known limitation as every prior session) — non-blocking, since the migration files are already committed and the live schema is independently confirmed correct; only the CLI's own tracking table is unreconciled. Updated `docs/SPRINT_RS_RELIABILITY_SECURITY.md`: RS-1 moved from `BLOCKED (needs Romy)` to `DONE 2026-09-10` (task table, detailed section, and the "Migration note" → "Migration confirmed live" paragraph), and the "Blocked on Romy" section rewritten to state nothing is currently blocked — RS-1 through RS-4 (all of Phase 1) are resolved (`e937567`). Also reconciled everything else sitting uncommitted from overlapping prior sessions: Session 146's `generation_resume_log.sql` migration (`e80cfc4`), the accumulated Sessions 144–149 `BUILD_TRACKER.md` narrative that had never been committed (`9acf624`), and a genuine `tsc` regression the pre-push hook caught — `eslint-local-rules/__tests__/require-supabase-error-check.test.ts`'s direct `ESLint` Node-API import needed types `eslint@8` doesn't bundle (my own earlier manual `tsc` check had run before that test file existed); fixed with `@types/eslint@8.56.12` (`102e385`), re-pushed clean. Pushed all of it to `origin/dev`, then opened and merged **[PR #2](https://github.com/ocdeployments/e2go/pull/2)** (`dev` → `main`, 49 commits spanning Gaps G-13 through G-25: RS-1–RS-5, RS-7, RS-10, plus Sprint DR resume-telemetry groundwork) as `5d25bb0`. GitHub Actions' `build-check` failed on the PR (`Error: supabaseKey is required` prerendering `/api/dashboard/consistency-sweep` and `/api/dashboard/package-manifest`) — investigated before merging: confirmed via `git diff main...dev` that neither failing route was touched by any of the 49 commits, and via `.github/workflows/security.yml` that CI only sets placeholder Supabase keys (no `SUPABASE_SERVICE_ROLE_KEY`), which both routes need at module scope — a pre-existing CI-environment gap, not a regression; `main` has no branch protection requiring the check; Vercel's own build (real secrets) passed. Merged anyway. **Romy ran `npx vercel --prod` and confirmed production is deployed.** Phase 1 of Sprint RS is fully shipped end-to-end (code, migrations, docs, merge, deploy). — Session 149: **closed RS-5 (Gap G-16) — gated unbound Supabase error destructuring behind an ESLint rule, and fixed a bracket-glob bug in the grandfather list before it could ship broken.** A prior session had already built the custom rule (`eslint-local-rules/require-supabase-error-check.js` — flags `const { data } = await supabase...` that omits `error`, since `supabase-js` never throws and this exact pattern caused the 44 silently-broken queries in `docs/SPRINT_S_SCHEMA_DRIFT.md`) and wired it into `.eslintrc.json` as error-severity in the strict zone (`src/middleware.ts`, `src/app/api/**`, `src/lib/**`) with a ~74-file grandfather list at warn (264 pre-existing violations, so the build isn't blocked catching up). **Bug found before it shipped**: six grandfather-list entries are real Next.js dynamic-route paths (`src/app/api/admin/promo-codes/[id]/route.ts` and five more under `generate/`) — minimatch, which ESLint's `overrides[].files` glob matching uses, reads unescaped `[...]` as a character class, not a literal bracket, so those six never matched their grandfather entry and silently fell through to the strict zone's `"error"` instead of `"warn"`. Confirmed empirically (direct ESLint runs, JSON severity output: `2` before, should be `1`) before fixing — escaped all six to `\[id\]`/`\[applicationId\]`/`\[jobId\]`, re-ran, confirmed `1` after. Added the sprint's required named test fixture, `eslint-local-rules/__tests__/require-supabase-error-check.test.ts` (5 cases via ESLint's Node API against the real `.eslintrc.json`, including a dedicated regression test pinned to this exact bracket-escaping bug so it can't silently regress). Full verification chain: `.eslintrc.json` JSON-valid; all six previously-broken files confirmed warn-severity; full-project lint (551 files) shows zero unexpected errors from the new rule (24 pre-existing unrelated errors untouched, 374 warnings including 364 from this rule); `npx tsc --noEmit -p .` clean; the 5-case test fixture passes; full `npx jest` green (26 suites/321 tests); the Exit criterion literally demonstrated (a throwaway strict-zone route file with an unbound destructure made `next build`'s ESLint pass exit 1 with the expected message, then deleted); `npm run build` clean (200 pages). `docs/SPRINT_RS_RELIABILITY_SECURITY.md` updated: RS-5's task-table row and detailed section now read DONE, in the same "RESOLVED" style as RS-7/RS-10. One commit-worth of changes staged but **not yet committed** — `.eslintrc.json` (bracket fix), `eslint-local-rules/` (new plugin + test, untracked from the prior session), `package.json`/`package-lock.json` (prior session's `eslint-plugin-local-rules` file-link devDependency, reviewed clean — single-line addition), `docs/SPRINT_RS_RELIABILITY_SECURITY.md` (RS-5 marked done). **Also present in the working tree, untouched this session and not mine to reconcile**: Session 146's `supabase/migrations/20260910130000_generation_resume_log.sql` (Sprint DR, blocked on Romy applying it) and the large pre-existing `BUILD_TRACKER.md` diff spanning Sessions 146–148 (RS-1, RS-10, Sprint DR) — left as-is. **Next up per the sprint doc's priority order:** RS-6 (Gap G-21, atomic increment for the simulator pack grant, migration, 0.5 eng-day) is the next TODO item in Phase 2, not started this session. — Session 148: **shipped RS-1 (Gap G-13) — turned the webhook dedup row into a claim, not a receipt — code and tests complete, BLOCKED on Romy applying the migration.** `processed_webhook_events` used to be written once, before any handler logic ran, so a Stripe redelivery of an event that partially failed last time was reported `duplicate: true` forever and never retried. New `status` column (`processing`/`completed`/`failed`, default `'completed'` for historical rows) makes the row a claim: inserted as `'processing'`, a `23505` conflict now looks up the existing row's status and only short-circuits on `'completed'` — `'processing'`/`'failed'` fall through and reclaim it. All 16 `captureApiError` call sites inside the event `switch` (across all 4 event types) renamed to a local `captureAndFail` wrapper that also flips a `handlerFailed` flag; the whole switch is now wrapped in try/catch; a finalize step at the end writes `status: handlerFailed ? 'failed' : 'completed'`. Response codes are unchanged (still always `200`/`{received:true}`) — flipping failed claims to `500` is RS-2's job, deliberately not done here. Safe to flag every switch-body error site as failure-triggering because every downstream write there is an idempotent `UPDATE ... WHERE id = ...`, never an `INSERT` — reprocessing on redelivery can't duplicate side effects. Three commits, one file each: `35570ae` (migration), `fd713c6` (`src/app/api/stripe/webhook/route.ts`), `048dc09` (new `src/app/api/stripe/__tests__/webhook-dedup-claim.test.ts`, 6 tests covering clean-completion, genuine-duplicate short-circuit, reclaim-from-processing, reclaim-from-failed, capture-error→failed, thrown-exception→failed). `npx tsc --noEmit -p .` and full `npx jest` (302/302) green at every commit. **Critical finding — the migration is NOT live**: a direct Supabase REST query (`GET /rest/v1/processed_webhook_events?select=status&limit=1`) returned `42703 column processed_webhook_events.status does not exist` — confirmed via the live-schema-is-truth rule, same sandbox-has-no-DB-password/no-pooler-egress wall Session 146 hit for its own migration. **Do not deploy the RS-1 code ahead of the migration** — it reads/writes `status` unconditionally and would break every webhook delivery. `docs/SPRINT_RS_RELIABILITY_SECURITY.md` updated: RS-1's task-table row and detailed section now read `BLOCKED (needs Romy)` (not DONE — unlike RS-10, whose migration had already landed live by the time it was marked resolved), and a new paragraph in "Blocked on Romy" spells out the exact fix (paste the migration's `ALTER TABLE` into the Supabase Dashboard SQL Editor) (`3157d13`). **Collision check against Session 146's concurrent Sprint DR work**: the sprint doc's own "Blocked on Romy" text warned to confirm DR's WIP state before touching the webhook route or `src/types/generation.ts`. Checked after the fact (should have been checked first) — Session 146's two commits (`279e8be` on `docs/SPRINT_DR_DELIVERY_RELIABILITY.md`, `8b3df1c` on `src/types/generation.ts`) don't touch the webhook route, and `git status --short` at both the start of this session and after all 3 RS-1 commits showed no uncommitted/untracked Session 146 changes to it either — no actual collision occurred, but check proactively next time before editing a file a concurrent session's own notes flag as shared risk. None of this session's 8 unpushed commits (5 RS-10 + 3 RS-1) were pushed; confirm with Romy first. **Next up per the sprint doc's own priority order**, once RS-1's migration is confirmed live: **RS-2** (wrap the event switch in an error boundary, Gap G-14) is now a small remaining task since RS-1 already added the try/catch and `handlerFailed` tracking it needs — just flip the final response to `500` when `handlerFailed` is true, plus a named `webhook-error-boundary.test.ts`. Then RS-3 (middleware fail-open, Gap G-15) and RS-4 (reconciliation cron backstop) finish Phase 1. — Session 147: **closed RS-10 (Gap G-19) — reconciled the two retention notices and shipped the three-email purge sequence.** Extracted `purgeExpiredFiles`/`sendRetentionReminders`/`sendRetentionCompletions` (plus `daysAgo`/`BUCKET`) out of `src/app/api/cron/data-retention/route.ts` into a new `src/lib/retention-cron.ts` — Next's typed-routes constraint (`.next/types/app/api/.../route.ts`) whitelists only `GET`/`POST`/etc. exports from a `route.ts` file, so the three business-logic functions had to become an importable lib module (matching the existing `retention-sequence.ts`/`partnership-hold.ts` convention) rather than staying inline. Added two new test files: `src/lib/emails/__tests__/retention-sequence.test.ts` (19 tests, the three retention emails' builders/senders) and `src/app/api/cron/__tests__/data-retention-hold.test.ts` (4 tests — confirms `purgeExpiredFiles` skips files whose application carries an active `retention_hold_at`, and that Module 1 / the privacy policy state the same 30/90-day schedule). Updated `docs/SPRINT_RS_RELIABILITY_SECURITY.md`'s RS-10 section, task-table row, and the stale "Blocked on Romy" note to RESOLVED/DONE, following the RS-7 precedent style. Five commits, one file each, `5a33e70`..`7fb7b85` — `npx tsc --noEmit -p .` clean and full `npx jest` green at every commit (pre-commit hook). Not pushed this session (branch `dev`); confirm with Romy before pushing. **Next up per the sprint doc's own priority order:** Phase 1 (RS-1 through RS-4 — "before the first ten clients, ahead of DR-3") is the only remaining TODO block ranked above everything else; RS-5/RS-6/RS-8/RS-9/RS-11/RS-12/RS-13 remain TODO behind it. — Session 146 (in progress, ended on a blocker — see full entry below): **began Phase 1 of Sprint DR (DR-1/DR-2/DR-7) after Romy resolved Decisions 2 and 3.** Decision 2: checkpointed resume for DR-1, with a hard new requirement — every resume attempt's outcome must land in a queryable table, not a per-incident alert. Decision 3: release-with-flag for DR-6 (deferred to its own session). Two commits landed clean on `dev` (`279e8be` sprint-doc decisions, `8b3df1c` `IN_FLIGHT_STATUSES` + resume-telemetry types). **Blocked before commit 3**: wrote `supabase/migrations/20260910130000_generation_resume_log.sql` but cannot apply it to live Supabase from this non-interactive session — the CLI has no cached DB password and `migration list`/`db push` hang waiting on one; no SQL-execution RPC exists in the codebase as a workaround. Per the live-schema-is-truth rule, no code depending on this table (the lookup helper, the cron route) should be written until it's confirmed live. **Romy: next session needs either the Supabase DB password supplied to the CLI, or you to paste the migration SQL into the Supabase Dashboard SQL Editor yourself** (same pattern as Sessions 138/139) — full SQL is in the migration file and in the Session 146 entry below. — Session 145: **closed the partnership checkout door, swept the Canada-shaped assumptions out of the intake, and documented the delivery-reliability gaps as a sequenced sprint.** The partnership branch on the results page was gated on `complete_partnership` — a legacy *payment_type* the quiz never writes — so every partnership applicant saw the solo price and a live checkout button while the pipeline withheld all six `*_p2` documents; now held server-side at both checkout routes with a 409 before any Stripe session. The Canada sweep found eleven files, not the two reported, including a Tab A citizenship question that offered Canada as its only option. Gaps G-01…G-12 now live in `docs/DELIVERY_RELIABILITY_GAPS.md` and their fixes as DR-1…DR-22 in `docs/SPRINT_DR_DELIVERY_RELIABILITY.md`. **All 22 commits (18 code + 4 doc) pushed to `origin/dev` as `28bc75e..340a6c8`** after a clean `tsc`, `npm run build`, 273 jest tests and 27 Playwright security tests. — Session 144: **recorded `cf8b44f` and closed the Stage 5 `elementPatterns` gap Session 143 deferred.** The map covered only 24 of the 45 required-element names in use; the other 21 resolved to an empty pattern array and so were reported missing regardless of content — and because a Stage 5 miss writes `quality_gate_passed: false`, which Gap 3 renders as a blocking "HELD FOR E2GO REVIEW", **eight document types could never be delivered at all** (`visa_category`, `nonimmigrant_intent` + `_p2`, `marginality_rebuttal`, `net_worth_statement`, `property_portfolio`, `declaration_spouse`, `gift_letter`). All 21 added and calibrated against the 15 real Session 143 documents; the check now fails open on an unmapped element with a CI drift test to catch the drift instead; `applicant_name` now matches the real name (the `/applicant/i` proxy was blocking `org_chart`, which names the applicant eight times and never says "applicant"). `cf8b44f` was Romy's own commit awaiting the early-access welcome-email send. Also corrected: Session 143's four commits are on `origin/dev`, not unpushed. 263 tests green, build clean; two commits on `dev`, unpushed. — Prior handoff (Session 143) below. Session 143: **ran the first real end-to-end LLM generation job against Session 142's rule book, verified Gaps 1–3 live, and fixed two bugs the run exposed.** Romy: *"run one real generation job now that it's pushed."* Ran a full 15-document generation job for `test-uk@example.com` (James Windsor, home care franchise). Found and fixed two defects along the way: (1) the case brief was missing `applicant_name`/`business_name` for this persona — `runGenerationPipeline()` now backfills them (plus `investment`) from the `applications` row and the `M3-F-02` answer when the case-brief JSON doesn't already carry them. (2) The quality-gate retry path was calling the raw Anthropic SDK with only the system prompt and the literal user message "Regenerate the document now." — no case brief, no Module 3 answers, no exhibit registry — so Claude correctly refused, and *that refusal text* was what got saved as `content_text`. Fixed by routing the retry through `callClaudeAPI()` with a full `GenerationPayload` (same function every other regeneration site already uses), carrying the failure instructions in `case_theory_brief`. Both fixed in `src/lib/generation-engine.ts` (commit `b30a8f4`); a related `.env.local` quote-stripping bug in `scripts/run-persona-generation.mjs` was fixed alongside it (commit `2376e29`). **Gap 3 (blocked-status delivery gate) verified live at both layers**: `GET /api/dashboard/package-manifest` (called with a proper `Authorization: Bearer` token — that route authenticates by header, not cookie session) returned `status:"blocked"` with specific `blockedReason` strings for 6 of 15 documents and `packageReady:false`; the real `/documents/[applicationId]` page, viewed by minting a magiclink session for the test account and injecting it client-side via `window.__e2go_supabase__.auth.setSession()`, correctly rendered red "HELD FOR E2GO REVIEW" badges with the failure reasons and a "REQUEST NEW DRAFT" button in place of certification. Gap 1 (voice/person correctness) confirmed live in the generated documents themselves (spouse/principal declarations first-person, qualifications/résumés third-person, cover letter officer-facing). **New finding, not yet fixed — a placeholder-leak defect that bypasses the Gap 3 gate**: several documents (Cover Letter, Non-immigrant Intent Statement, Fund Flow Chronology, Consolidated Net Worth Statement, DS-156E/DS-160 Reference, Principal Applicant Résumé) contain literal unfilled template markers (`[Date]`, `[Consulate address]`, `[DATE REQUIRED]`, `[AMOUNT NOT PROVIDED]`, `[CONFIRM WITH APPLICANT]`, etc.) that the UI flags as "CRITICAL" — but that check is independent of `quality_gate_passed`, so a document can carry leaked placeholders and still show "AWAITING CERTIFICATION" (confirmed on the Résumé). This is a real gap in the "no unreviewed document reaches the client" goal and needs a decision on scope/priority. Also newly confirmed live: the `gift_letter` quality-gate false positive (correctly writes "NOT APPLICABLE — No gift funds used" but still gets held for "missing donor_name/gift_amount/irrevocability"), plus broader calibration issues across the real run (page-length overages on 7+ docs, missing legal disclaimers on 5, missing required elements on 6+, Cover Letter missing the required "direct and develop" phrase, Business Plan flagged too brief) — valuable first-real-run signal, out of scope for this session's fix. Hit the `.next` dev/build cache collision a second time this session (a forgotten lingering `npm run build` background process corrupted the shared directory while the dev server was serving) — same fix as before (`preview_stop` → `rm -rf .next` → `preview_start`), plus a new safeguard applied: stop the dev server before running any further production build in-session. `npx jest` 18 suites / 247 tests green (pre-commit hook, twice); `npm run build` clean. Two commits on `dev` (`b30a8f4`, `2376e29`) — **not yet pushed**, pending Romy's decision on the deferred findings above. — Prior handoff (Session 142) below. Session 142: **extracted the universal system prompt into one shared file and closed four document-generation "rule book" gaps.** Romy: *"extract the universal prompt into one shared file, and close the other gaps to make it efficient and then do regression testing."* (1) The `## UNIVERSAL SYSTEM PROMPT` block was copy-pasted into 21 `prompts/v1/documents/*.md` files and had drifted (line counts 18–74, a `until it until it` typo in `business_plan.md`, only 3 files carrying the full 8-principle version) — extracted to `prompts/v1/_universal_system_prompt.md` (canonical, typo-free, all 8 principles); `loadPrompt()` now prepends it and defensively strips any stale in-file copy; the 21 doc files keep only their genuine per-doc voice/person deltas; new `universal-prompt.test.ts` guard blocks re-divergence. (2) Every generation job now writes one no-schema `document_generation_log` row (`stage:'rulebook_version'`) stamping `RULEBOOK_VERSION` + a sha256 hash of the whole prompt corpus, so old vs new output is always attributable. (3) Legal-boundary gate: tightened `LEGAL_CONCLUSION_HARD_RE` to word-boundary matching with a negation guard (kills false positives on "eligible for SBA financing" etc.), and made the manifest / certify / download paths treat `quality_gate_passed === false` as a **blocking** `'blocked'` status ("HELD FOR E2GO REVIEW") — a document that fails the legal-boundary check can no longer reach the client unreviewed. (4) `validateForGeneration()`'s `fund_sources_sum_mismatch` check was dead code (no per-source amounts were ever collected) — added a per-source USD amount sub-form to Module 3 → Source of funds, stored under companion key `M3-F-05-AMOUNTS` (JSON object), and wired the check to reconcile the sum against `M3-F-02` within a $1 tolerance. Regression: `npx jest` 18 suites / 247 tests green; `npm run build` clean (192/192 pages); live E2E of the Gap 4 UI on `test-france@example.com` (renders, mismatch message, clears, persists with correct answer-key shapes); direct-DB schema checks for Gaps 2 & 3b. 13 commits, one file each, `258167e`..`959b2ba`, pushed to `origin/dev`, pre-push green (jest + build + 27 Playwright security tests, 48.1s). **Note:** a full LLM generation job was not run end-to-end — the sandbox has no egress to Upstash Redis so routes that hard-await redis 500 locally; judged adequately covered by 247 tests + clean build + direct schema checks. — Prior handoff (Session 141) below. Session 141: **built a public early-access lead-capture form for the Facebook Marketplace group.** Romy: *"I would like to create a form that I can send to a Facebook group which essentially lets people fill out their basic information like email, name, country of residence, and how quickly they are looking to file their e2k... The information of course needs to be recorded and should be available to me as prospective clients in the future."* Built: new `early_access_leads` table (migration **not yet run — Romy must execute it**, see Session 141 entry below for the exact SQL), a public unmarked form at `/early-access` (name, email, country, filing timeline, honeypot anti-bot field), `POST /api/early-access` (rate-limited 5/60min per IP, validates + upserts on email), and `/admin/early-access` (admin-only listing page + the shareable link, nav-linked from `/admin`). `npm run build` clean, all three new routes verified in-browser (form submits correctly, live-tested `PGRST205` table-missing error confirms the code path works end-to-end and is only blocked on the pending migration). — Prior handoff (Session 140) below. Session 140: **named the operating entity everywhere the company is legally identified.** Parent company is **Prodigal Son IT Solutions LLC** (Texas LLC), d/b/a e2go. Fixed the Terms (was "E2Pathway Inc., a Texas limited liability company" — wrong entity + self-contradictory), Privacy Policy, `Footer.tsx`, and the homepage footer to name `Prodigal Son IT Solutions LLC, d/b/a e2go`. New `src/lib/emails/company.ts` centralises the legal identity (`COMPANY_LEGAL_NAME`, `COMPANY_DBA`, `companyFooterLine()`); wired into the shared email footer (`base-template.ts`) + the two private footers (`clock1`, `clock2`) + the `results-email` plain-text footer. **No street address anywhere public** — Romy does not want the home address exposed and US law does not require one on a website/privacy policy; `COMPANY_POSTAL_ADDRESS` is left `''` with a TODO — transactional mail is CAN-SPAM/CASL-exempt from the address rule, so footers print the name alone until a registered-agent / virtual-mailbox address is filled in (required before any *commercial/marketing* email send). 9 commits, one file each, `982a5df`..`76822f8`. **Romy still to do (non-code):** file the Texas Assumed Name Certificate (Form 503) for "e2go"; set Stripe legal business name + statement descriptor; move vendor billing (Supabase/Vercel/OpenRouter/Resend/Groq) to the LLC; business bank account + EIN in the LLC name; insurance issued to "Prodigal Son IT Solutions LLC"; fill `COMPANY_POSTAL_ADDRESS` before marketing email. — Prior handoff (Session 139) below. Session 139: **built the deferred P2 hardening — document-access audit log, identity-doc `extracted_json` redaction, per-file delete-now, and the CLAUDE.md reconciliation.** (1) New `document_access_log` table (migration `20260907140000`, run + reconciled) + `src/lib/document-access-log.ts` fire-and-forget helper (service-role write, errors to Sentry, never throws). Instrumented `view`/`delete` on `GET`/`DELETE /api/documents/[documentId]`, `extract` on `/api/documents/extract` + `/api/fdd/extract`, `parse` on `/api/apply/parse-document`, `delete` on the new `DELETE /api/uploaded-documents/[id]`. No IP capture (nothing captures IPs today; `user_id`+`created_at` answers who/when). RLS: owner reads own trail, service role is sole writer. `user_id` FK is `ON DELETE CASCADE`. (2) Blanket `extracted_json` strip confirmed unsafe (load-bearing for simulator routes + `document-comprehension-engine`) — scoped to identity doc types only (`passport, birth_certificate, marriage_certificate, national_id, government_id, drivers_license`), `fields_accepted > 0`, 1-day grace. New `redactAcceptedIdentityDocs()` pass in the daily `data-retention` cron overwrites `extracted_json` with `{_redacted:true,_redacted_at,_reason}`; accepted fields stay in `answers`. `_redacted` guards added to all three consumers (`summarizeExtractedJson`→'', prep-kit `describeExtractedJson`→null, comprehension-engine→excluded from LLM input). No migration (marker lives in the existing jsonb column; that table stores no files). (3) `DELETE /api/uploaded-documents/[id]` (user-scoped; the existing delete route only covered `application_documents`) + a delete button in the case-profile `ExtractionTransparencyPanel` (confirm dialog, inline error, `reloadProfile()` on success). Corrected stale upload-screen privacy copy to the real 30-day-post-package / 90-day-max window + immediate-delete option. `docs/DATA_RETENTION_POLICY.md` updated (identity `extracted_json` exception row, `document_access_log` schedule row, operational checklist). (4) **Option A chosen** for the CLAUDE.md reconciliation — global `~/.claude/CLAUDE.md:185` "never store sensitive documents" left untouched; a scoped `## DOCUMENT STORAGE` carve-out added to the project `CLAUDE.md` (identity docs never stored; financial/business docs stored transiently per `docs/DATA_RETENTION_POLICY.md`; "do not fix this by removing the bucket"). 15 commits, one file each, `ac5e55c`..`f2c5f9d`. Migration `20260907140000` run in the Supabase SQL Editor by Romy, PostgREST-probed (200, all 10 columns), `migration repair --status applied` + `db push --dry-run` clean ("Remote database is up to date"). — Prior handoff (Session 138) below. Session 138: **built the data-retention layer and aligned the privacy policy with reality (P0/P1/P2 from Romy's security directive).** No law imposes a minimum retention on the documents e2go handles; the actual violation was "keep forever + a deletion email that deleted nothing." Fixes: (1) `scripts/verify-storage-buckets.mjs` — confirms `application-documents` is private (it is; `uploaded-docs` bucket does not exist, proving the intake path never stored files). (2) New `/api/cron/data-retention` (daily 04:00 UTC, 4th entry in `vercel.json`) — purges Storage objects + `auth.users` row 30 days after account deletion (making the deletion email truthful for the first time), deletes raw uploaded files 30 days after the document package is generated or 90 days after upload (whichever first), stamps `file_purged_at`, and logs dormant accounts (24 mo, report only). (3) Migration `20260907120000_document_retention.sql` adds `file_purged_at` to `application_documents` + `fdd_analyses` — **Romy must run this in the Supabase SQL Editor.** (4) `/api/documents` now rate-limits (`parse-doc` profile) and rejects identity-doc types (passport/birth cert/etc.) — those go through the memory-only intake path. (5) Both extract routes skip purged files with a re-upload message. (6) `docs/DATA_RETENTION_POLICY.md` — written schedule, legal basis, sub-processor table. (7) Privacy policy: removed the false "re-download" and "access is logged and audited" claims, replaced the single wrong "90 days after visa outcome" retention line with the real 6-part schedule, disclosed Google Gemini (via OpenRouter) as a document-content sub-processor, and stated passports are never stored as files. **Walk-back confirmed:** Romy was right that E-2 needs source-and-path-of-funds — financial extraction schemas are unchanged (they already capture institution names / balances / source narrative and never full account numbers). 10 commits `7c85774`..`c438512`, pushed to `origin/dev`, pre-push green (jest 191 + build + Playwright 27). Deferred P2: document-access audit log; `extracted_json` PII dedupe after fields are accepted; per-file "delete now" button in the UI (backend `DELETE /api/documents/[id]` already exists); reconcile CLAUDE.md "never store sensitive documents" with the `application-documents` bucket (Romy's call). — Prior handoff (Session 137) below. Session 137: **finished the `/pricing` page — concrete card copy, gold hover treatment, and the Session 132 pricing-pivot drafts landed.** (1) Rewrote all four `PRICING_TIERS` card copies in `src/lib/pricing-tier.ts` so every bullet names a real deliverable — document names from `docx-package-constants.ts`, the 15 denial factors from the gap engine, the four E-2 tests from `fdd-scoring-engine.ts`, Census-ACS competition/demographic-fit scoring from `market-analysis-pdf.ts`, the 5 mock interviews + per-question feedback from `simulator-engine.ts` — and the Visa Ready "$180 less than à la carte" math is verified ($990+$390+$290−$1,490). (2) Added a gold hover treatment to `PricingCard.tsx` — `-translate-y-1.5` lift, `#C9A84C` border, `0_0_50px` gold glow, a top hairline that sweeps in, title turns gold, CTA fills gold; all `transform`/opacity only, `motion-reduce` guarded. (3) Committed the Session 132 drafts Romy had been holding (`results/page.tsx` rebuilt around Foundation, à-la-carte `modules/page.tsx` retired to a `/pricing` redirect) plus the prior-session `/pricing` restructure into packages + add-ons and the friendlier add-on-gating 403 messages. (4) Committed Romy's own uncommitted work: `docs/MARKETING_STRATEGY.md` + its DOC_INDEX row, three Stripe/promo setup scripts, and a `.gitignore` rule for `/scratchpad/`. 12 commits, one file each, `7249f4d`..`778698a`; pushed to `origin/dev` (two pushes, pre-push green both times). Romy ran `npx vercel --prod` by hand — **live** (Claude's deploy command is blocked by the auto-mode classifier). Stray `scratchpad/quiz_sessions_investment_currency_usd_only.sql` was a byte-identical older draft of the tracked migration — deleted. — Prior handoff (Session 136) below. Session 136: **closed out the migration/history work and audited the Session 132 pricing-pivot scope.** (1) Currency migration + migration-history reconciliation recorded as done and pushed (`c8332d7`, `5243fd3`). (2) Deleted the dead `src/types/payments.ts` (`cba7268`, pushed). (3) Audited what "item 3 / pricing pivot" actually still needs: the BUILD_TRACKER scope notes were stale — `create-checkout` + `checkout/initiate` are already the 4-tier USD model with clean env var names (no `STRIPE_PRICE_COMPLETE_PARTNERSHIP`, no `interview_prep` price-ID collision, no mode mismatch), and CAD price logic is already gone (Session 135). Real remainder: the two uncommitted Session 132 drafts (`results/page.tsx`, `modules/page.tsx`) — reviewed, `tsc` + `npm run build` clean with them in place, **Romy chose to hold both uncommitted** pending a look at the rendered pages; three `NEXT_PUBLIC_*`/`RENEWAL` Stripe env vars missing from local `.env.local` (verify in Vercel, not edited here per the .env.local safety rule); new budget tier **deferred by Romy** ("skip it for now"). No code committed for item 3 this session. — Prior handoff (Session 135) below. Session 135: **executed the Session 134 handoff sprint + the CAD→USD conversion.** (1) All six "old model leaking into live UI" items fixed (promo-code form, admin tier-override panel, revenue report maps, pricing-grid `UTILITY_TIERS`, checkout success page, two admin label maps). (2) All CAD currency amounts converted to USD and every money input now visibly labelled USD — no FX API, just labels + a migration tightening `quiz_sessions.investment_currency` to USD-only. Also fixed a latent bug where a CAD net-worth value was silently mis-scored against the USD investment figure in the desperation-ratio signal. 16 commits, one file each, `e0830c4`..`4a07459`; `npx tsc --noEmit` + `npm run build` clean; 191 jest tests pass. **Pushed to `origin/dev`.** Romy to do: apply `supabase/migrations/20260906130000_*.sql`; sanity-check the two hypothetical USD figures in `ComparisonSection.tsx` (consultant $2,500–$6,000, attorney $6,000–$15,000). Session 132's broader pricing-pivot scope (uncommitted draft rewrites in `results/page.tsx` + `modules/page.tsx`) still stacked behind this — untouched. — Prior handoff (Session 134) below. Romy's own words (voice-dictated, garbled by transcription — decoded below): *"I asked you to delete the [old pricing] model and concentrate only on the USD [model]. That is it. And the [promo codes] are still reflecting the [old pricing] model. We moved away from that already... This is we are going in circles."* Investigation (read-only, nothing changed yet) confirmed the core complaint: `src/lib/pricing-tier.ts` itself is already correctly USD-only on the new 4-tier model (`foundation` $990 / `investor_ready` $390 / `interview_prep` $290 / `visa_ready` $1490), but several **customer- and admin-facing surfaces built in earlier sessions never got updated to match** and still show/accept the retired 7-SKU tier IDs (`complete`, `complete_partnership`, `interview_prep_partnership`, `fdd_intelligence`, `fdd_intelligence_loyalty`) — most concretely, the promo-code admin page built last session (`PromoCodeForm.tsx`) lets Romy scope a discount to tiers that no longer exist for sale. Full file-by-file punch list, and which old-tier references are legitimate legacy-support code that must NOT be touched, is in the **Session 134** entry directly below. Also found genuine CAD/Canadian-currency remnants (DB constraint, a net-worth field, a live application question, marketing copy) — also listed below, one of which requires Romy to run SQL herself per the standing no-direct-SQL-on-production rule. Session 132's broader pricing-pivot scope (uncommitted draft rewrites already sitting in `results/page.tsx` and `modules/page.tsx`) is unstarted and still stacked behind this.
...
- [x] BC-12 final step: merge PR #7 (`rotate()` fix), re-run `db-backup.yml`, confirm all 4 steps green end-to-end — **DONE**, verified Session 153: PR #7 merged 2026-09-13T05:34:43Z, `Database Backup` green Sept 13 and Sept 14
- [ ] BC-12 follow-on: the *restore* drill below is the part that actually proves the backup works — still outstanding
...
## Session 153 — Session-start audit: WHAT IS CURRENTLY LACKING (September 14, 2026)

**Branch:** dev. Read-only audit performed at session start, verified against the live repo, `git`, and the GitHub API — not against the docs. Recorded here so the next session does not have to rediscover it. Nothing in this section is fixed yet.

### Broken right now

- **`Security Gates` CI is RED on `main`, and has been on every scheduled run — including today's (2026-09-14T15:23).** Four of five jobs pass (`type-check`, `build-check`, `secret-scanning`, `dependency-audit`). The one that fails is `trufflehog`, with `##[error]BASE and HEAD commits are the same. TruffleHog won't scan anything.` Cause is a workflow config bug, not a leak: `.github/workflows/security.yml:86-98` passes `base: ${{ github.event.repository.default_branch }}` and `head: HEAD`, which resolve to the *same commit* on `push`-to-`main` and on `schedule`. The diff-scan mode only makes sense on `pull_request`. **Practical effect: the verified-secret scan has effectively never run on `main`, and the repo has been showing a red daily badge that everyone has learned to ignore.** Fix direction: branch the args by event — diff mode (`base`/`head`) for `pull_request`, full-repo filesystem scan (no `base`/`head`) for `push`/`schedule`.

- **`markTokenUsed()` in `src/app/actions/verify-token.ts:38` has ZERO callers.** `grep -rn "markTokenUsed" src/ tests/` returns only the definition. Consequences, in order of severity:
  1. `email_verifications.used` is never set to `true`, so the `.eq('used', false)` filter in `verifyToken()` never excludes anything. **Every results link is reusable for its full 24-hour window.**
  2. The results email tells the recipient *"Your link opens once"* (`src/lib/emails/results-email.ts`). That is a **factually false statement in client-facing copy** — it is not a weak line, it is a claim the system does not implement.
  3. Do **not** "fix" this by simply wiring `markTokenUsed()` into the verify path. Genuinely single-use links break on corporate mail scanners and inbox link-prefetchers, which fetch every URL in a message before the human ever clicks — the user then gets "this link was already used" on their first real click. Either (a) drop the single-use claim and keep the 24h window (copy change only, cheapest), or (b) implement single-use properly with a POST-confirm interstitial or a grace window, and only then keep the claim.

### Out of sync / unmerged

- **`origin/main` is 17 commits behind `origin/dev`** (`git rev-list --left-right --count origin/main...origin/dev` → `17  0`). The Sept 13 Playwright/Turnstile test fixes and the launch-readiness checklist above are on `dev` only. Zero unpushed commits on `dev`.
- **An uncommitted compliance fix is sitting in the working tree.** `src/app/HomeClient.tsx` + `src/components/landing/SectionNav.tsx` remove the three placeholder testimonials ("Marco T., Italy", "Aisha K., United Kingdom", "David L., Canada") and the "Reviews" nav entry, citing FTC 16 CFR Part 255 and the 2024 Rule on Consumer Reviews and Testimonials (16 CFR Part 465). This is a **legal-exposure fix that is currently protecting nobody, because it is not committed and not deployed.** Also uncommitted: `.gitignore` (adds `/demo-studio/build/`) and untracked `demo-studio/` (4 `.mjs` files).

### Checklist items that are further along than section A claims

- **`BC-12 final step` is effectively done.** PR #7 (`rotate()` crash on empty R2 prefix) was **merged 2026-09-13T05:34:43Z**, and `Database Backup` has since run green on Sept 13 *and* Sept 14. Mark the merge/re-run part done; the **restore drill** (pull a real R2 backup, `pg_restore` into a scratch DB, confirm queryable) is still genuinely outstanding and is the part that actually proves anything.

### Documentation drift

- **`CLAUDE_CONTEXT.md`'s ROUTE MAP is badly stale.** It claims "60 routes as of June 19, 2026". Actual count today: **110 page routes** (`src/app/**/page.tsx`) and **124 API routes** (`src/app/api/**/route.ts`). A context file that understates the app by ~45% is worse than no route map, because it is read as authoritative at session start.

### Blocked on Romy (nothing an agent can do)

- **`COMPANY_POSTAL_ADDRESS` in `src/lib/emails/company.ts` is still `''`.** Transactional mail is fine without it. **Any commercial/marketing send is not** — CAN-SPAM and CASL both require a valid physical postal address. A registered-agent or virtual-mailbox street address is sufficient and does not have to be a home address. This is a hard gate on the first marketing email, and it also constrains how persuasive the results email is allowed to get before it reclassifies as commercial (see the results-email review in this session).
- **MCP availability:** `lazyweb` fails to connect (`ECONNRESET`, both the direct and plugin instances); `figma` requires an OAuth authorization that cannot be completed from a non-interactive session. Design-research steps in the global build process are unavailable until these are fixed by Romy.
...
**Romy's ask, staged explicitly across several turns, each gated on the last:**
1. Run a thorough multi-perspective gap analysis of business-continuity/disaster-recovery risk — explicitly *not* to start planning fixes yet.
2. Score that report yourself and find its own weaknesses before treating it as final.
3. Only then design a mitigation plan — and score that too — then report back.

**Phase 1 — the audit.** Five hats run as parallel read-only subagents against the live repo: SRE, Database Optimizer, Security Architect, Compliance Auditor, Incident Responder. Produced 16 findings, later numbered G-1 through G-16 (1 CRITICAL, 5 HIGH, 7 MEDIUM, 3 LOW).

**Phase 2 — self-critique.** Scored the report's own rigor, then went further than reasoning-only critique at Romy's prompt ("anything else you want to check before calling this seriously good?") and ran real, direct verification instead of leaving items as flagged-unknown:
- `vercel env ls production` — confirmed **no `DATABASE_URL`, `POSTGRES_URL`, or `SUPABASE_DB_URL` exists anywhere** in the app's 31 production env vars. The app has never had a code path to run its own Postgres backup (`pg_dump` or otherwise); recovery today depends entirely on Supabase's own managed backup product, whose plan tier and PITR window are dashboard-only and unconfirmed (no `SUPABASE_ACCESS_TOKEN` available to this session or the unlinked local CLI).
- `gh api repos/ocdeployments/e2go` and the branch-protection/rulesets endpoints — confirmed the repo is **PUBLIC**; `secret_scanning` and `secret_scanning_push_protection` are both **enabled** (positive finding); `dependabot_security_updates` is **disabled**; both `main` and `dev` have **zero branch protection and zero rulesets** (classic API 404 on both, rulesets endpoint returns `[]`).

These four facts were new information, not previously confirmed in the codebase-only read.

**The CRITICAL finding (G-1):** `src/lib/llm-client.ts` and `generation-engine.ts`'s extraction/FDD fallback chains route user document text to **Zhipu AI** (`z-ai/glm-5.2`), **Xiaomi** (`mimo-v2.5`/`-pro`), and **Anthropic directly** (bypassing OpenRouter) — none of which are disclosed anywhere; the Privacy Policy and `docs/DATA_RETENTION_POLICY.md` name only Google/Gemini via OpenRouter as a document-content sub-processor. This is a live compliance gap (every historical fallback invocation already sent data to an undisclosed party), not a theoretical risk.

Full 16-item register, each tagged CRITICAL/HIGH/MEDIUM/LOW with file/line citations: `docs/BUSINESS_CONTINUITY_GAPS.md`.

**Phase 3 — mitigation sprint**, opened only after Romy's explicit "record this, build the sprint, score it, report back." Modeled on `docs/SPRINT_RS_RELIABILITY_SECURITY.md`'s existing template (task table, Gates, Legend, "Blocked on Romy," self-score). Chose sprint code **BC** (Business Continuity) — deliberately distinct from the existing **DR** code, which in this repo already means *Delivery Reliability*, not Disaster Recovery, to avoid a naming collision. `docs/SPRINT_BC_BUSINESS_CONTINUITY.md` holds 22 tasks (BC-1 through BC-22) across 4 phases:
- **Phase 0 (this week):** BC-1 (decision: disclose vs. remove the undisclosed LLM sub-processors, G-1), BC-2/BC-3 (wire an active alert onto `payment-reconciliation.ts` mismatches and `generation-engine.ts`'s silent top-level `fail()`, both G-7, 0.5 day each), BC-4 (Romy verifies real Supabase PITR/plan-tier/bucket-versioning status in the dashboard, G-2, 15 min).
- **Phase 1 (before the next 25 clients):** branch protection on `main`/`dev` (BC-5/BC-6, G-5), enable `dependabot_security_updates` (BC-7, G-6), content-based identity-document detection instead of client-declared type (BC-8, G-4, 1.5 days), fix `outcomes_consent` — persist server-side + un-bundle from mandatory disclaimers (BC-9, G-3, 1 day), fix `health-watchdog`'s stale watchlist (BC-10, G-7), stop defaulting ops alerts to a personal Gmail + fail loudly not silently (BC-11, G-8), and BC-12 — an independent logical backup, sized entirely by BC-4's answer, currently `BLOCKED (needs BC-4)`.
- **Phase 2 (this quarter):** proxy login/signup through an app route for CAPTCHA/rate-limiting + fail-closed (BC-13, G-12), wire the never-fired `'download'` access-log action (BC-14, G-11), document the `retention_hold_at` policy exception (BC-15, G-9), add a soft-delete/archival window before hard deletes (BC-16, G-10), consolidate two hardcoded `TERMS_VERSION` constants (BC-17, G-14), write 3 runbooks for undrilled failure classes — leaked key, Stripe outage, Supabase outage (BC-18, G-13), extend `chaos-drills.mjs` with a payment-reconciliation drill (BC-19, G-13).
- **Phase 3 (opportunistic):** link `supabase/migrations/` via `config.toml` (BC-20, G-15), sample the ~23 unreviewed API routes + 6 admin routes (BC-21, G-16), revisit the single-shared-Supabase-project decision now that a backup story exists (BC-22).

**Sprint self-score** (same discipline the report itself was held to): Coverage of the gap register 9/10 (every G-# maps to a BC-#), Sequencing logic 8/10 (flagged BC-8 as a judgment call — HIGH severity but placed in Phase 1, not Phase 0, since it needs a mislabeled upload to trigger), Effort estimates 6/10 (explicitly called rough-order-of-magnitude, not calibrated like Sprint RS's session-over-session numbers), Decision-vs-code discipline 9/10 (9 of 22 tasks are `decision`/`manual`/`infra` rather than code this agent could just implement), Independent verifiability 7/10 (every Phase 0/1 task traces to something checked directly this session or a prior one — the one placeholder is BC-12's cost, which can't be known until BC-4 answers). **Overall verdict:** a solid starting sequence, not a finished project plan — its biggest weakness is that BC-1 and BC-4, both Phase 0, could materially resize everything downstream (BC-1 could become "remove three fallback models" instead of "update two docs"; BC-4 could make BC-12 disappear entirely or become the sprint's single largest task). Recommended running Phase 0 in isolation before committing to Phase 1's effort numbers as a block.

**Known internal-consistency note, not yet reconciled with Romy:** this register's G-1..G-16 numbering is a fresh, locally-scoped sequence, not a continuation of the cross-sprint G-01..G-25 numbering already used by Sprint DR (G-01–G-12) and Sprint RS (G-13–G-25). Worth asking Romy whether she wants global numbering going forward — not corrected unilaterally here since it's a naming-convention call, not a substantive fix.

**Nothing built or committed this session** — planning and docs only, per Romy's own staged sequencing. Both new doc files and this entry are the sum of the session's output; every BC-# task starts at `TODO` per this project's "no task is Done on reasoning" rule.

**Next agent — start here:** don't start Phase 1 work yet. Surface BC-1 (sub-processor disclosure vs. removal — a legal/product call) and BC-4 (a 15-minute Supabase dashboard check only Romy can do) to Romy first; BC-2 and BC-3 can start immediately in parallel since neither depends on a decision, and they close the two most consequential silent-failure paths in the codebase (payment reconciliation and the generation pipeline's own failure handler). Hold every Phase 1 effort estimate as provisional until BC-4's answer lands.

**Update, same session:** BC-2 and BC-3 are now done — see the Session 152 (cont.) summary at the top of this file for the commits and verification. Phase 0 is now blocked purely on Romy (BC-1 decision, BC-4 dashboard check); nothing else to build until then.

**Update, same session (2):** BC-10 is now also done (G-7's `health-watchdog` cron watchlist fix) — see the newest Session 152 (cont.) summary at the very top of this file for commits, verification, and the `rebuild-profiles` finding surfaced along the way. Not pushed. Next up: BC-11 (Phase 1).

**Update, same session (3):** everything is now fully caught up and pushed. The DR-20 doc-update commits (`2dcd263`, `d3e3c91`) went to `origin/dev`, and `origin/dev` → `origin/main` merged clean as **[PR #4](https://github.com/ocdeployments/e2go/pull/4)** (`025a9aa`) — local `main` was stale (47 commits behind) and has been fast-forwarded to match. No further push/merge action needed as of this update; see the top-of-file "Last Updated" entry for detail, including the DR-20 pilot-cell login given to Romy for direct inspection.

---

## Session 151 — DR-20 real-pipeline delivery-matrix plan finalized and approved, no implementation yet (September 11, 2026)

**Branch:** dev. **Docs-only session, nothing committed.** Two files edited: `docs/SPRINT_DR_DELIVERY_RELIABILITY.md` (DR-20 section + Phase 8 effort row) and this file. No code, migration, or script was written.

**Romy's instruction, in order:** audit whether the existing synthetic test data is complete enough to run DR-20 (the 36-cell delivery test matrix) for real; produce a plan for what complete data would look like *before* building or fixing anything; only once real data exists, test the app end-to-end (intake → analysis → generation → review) in the browser looking for UI/UX breakage.

**The audit (read-only, confirmed before any plan was drafted):** the four existing seeded accounts (`seed-test-profiles.mjs` → Canada/France/UK, `seed-partnership-fixture.mjs` → Partnership) are not adequate. Only one of the four has a fully populated Module 3 answer set; none populate `case_brief_json` enrichment, `applicant_voice_profile`, `case_theory`, `document_intelligence`, or `followup_responses`; `generation-engine.ts`'s own comments call `case_theory`'s absence "the single most consequential missing input for narrative quality" — so this isn't cosmetic. None of the four cover independent business type, partial funding, or any of the three nationalities DR-20 needs.

**The plan went through two rounds of expert self-review at Romy's request before she'd approve it**, and both caught real design flaws rather than nitpicks:
1. The first draft hand-injected `case_brief_json`/`case_theory`/`document_intelligence` as fixture rows. But real producers already exist and are wired into the normal flow — `src/app/api/answers/route.ts` calls `buildCaseIntelligence`/`generateCaseTheory` in `case-intelligence-core.ts`, and `src/app/api/analysis/run/route.ts` writes `case_brief_json`. Faking these would have skipped the exact stage DR-20 exists to test. Revised design derives all three by calling the real routes.
2. There is no separate dev/staging Supabase project — `vercel env pull` confirmed Development and Production both point at `cziphinlzfnlqlvynwnm`. Romy chose to keep the one project rather than provision a new one, which meant a closer look at blast radius: a synthetic partnership fixture's fabricated `payments` row is read by `health-watchdog`'s `isPaidUser()` (could page Romy over a test job), `admin/revenue/page.tsx` (would inflate the revenue dashboard she actually reads), and `payment-reconciliation.ts` (would look exactly like the Stripe-ledger anomaly that job exists to catch). **Decided:** a new `payments.is_test_fixture boolean not null default false` column (idempotent `ADD COLUMN IF NOT EXISTS`), excluded via one shared helper — not three copies — from those three readers only; deliberately *not* excluded from `entitlements.ts`/`case-profile.ts`/`generate/start/route.ts`/`gap-analysis/page.tsx`, since those are the actual `isPartnership` gating logic the partnership cells exist to exercise.

**Romy's third round of feedback was a genuine question, not another rescore request:** *"what will we have in data once this plan is approved and what will be able to test, what is the test plan."* Answered by adding a data-provenance table (which rows are real-pipeline output vs. necessary fixture, e.g. `auth.users`/`payments`/`family_members` stay fixture since no real signup/Stripe/family-intake flow is in scope, while `case_theory`/`case_brief_json`/the generated package are fully real) and an explicit "what this run does and does not test" list (does test: completion, duration, cost, file-count-vs-tier-promise, content sanity, UI/UX; does not test: the `property_portfolio`/`financial_assets_portfolio` conditional docs, since the funding axis holds the fund-source string constant specifically to avoid contaminating other cells; does not fix: the 3 already-known bugs below) directly into the plan file. That answer is what got the plan approved.

**Three unrelated bugs found during research, flagged only per Romy's decision — not fixed as part of DR-20:** a stale `M3-A-08`/`M3-A-09` field mapping reused across 3 files, `M3-F-09`'s LLC-name gate checking the wrong field, and `archetype` never being persisted to `case_brief_json`. Fixtures will reproduce these exactly as real users hit them — that's intentional, not an oversight.

**Full approved design:** `.claude/plans/wobbly-bubbling-plum.md`. Summary — 36 cells (solo/spousal/partnership × franchise/independent × Japan/Canada/South Korea × funded/partial-funding, nationalities chosen by Romy from genuine E-2 treaty countries). Per cell, seed only what has no real producer (the ~222-key Module 3 answer set as individual authenticated POSTs to `/api/answers` — the route takes one key/value pair per call, so this is ~8,000 HTTP calls across all 36 cells before generation even starts — plus documents, a voice sample, and a direct `payments`/`family_members` insert for partnership/spousal cells). Everything else — `case_theory`, `document_intelligence`, `case_brief_json`, the generated package — comes from real calls to `/api/analysis/run` and `/api/generate/run`, authenticated via a server-minted magic link exchanged for a session, the same technique `run-persona-generation.mjs` already uses. Both new scripts need per-cell checkpointing/resume given the call volume, and a cost circuit breaker (`--max-cells`, refuses to exceed without `--confirm-full-run`) since a mistaken re-run could double the ~$300–500 estimated LLM spend.
...
## Session 148 — RS-1 (Gap G-13) shipped: webhook dedup row is now a claim — BLOCKED on Romy's migration apply (September 10, 2026)

**Branch:** dev. Three commits, one file each — `35570ae` (migration), `fd713c6` (route.ts), `048dc09` (test file), plus `3157d13` (sprint-doc status update, 4th commit, docs-only) — **not yet pushed**; confirm with Romy before pushing. `npx tsc --noEmit -p .` clean and full `npx jest` (302/302) green at every commit (pre-commit hook).

**What RS-1 was:** Gap G-13 — the webhook dedup row (`processed_webhook_events`) was a receipt, not a claim. It was inserted once, before any handler logic ran, and never touched again, so a Stripe redelivery of an event whose handler partially failed the first time was reported `duplicate: true` forever and silently never retried.

**The fix:** a new `status` column (`processing` / `completed` / `failed`, `CHECK` constraint, defaults to `'completed'` for historical rows since they predate the column and were written under old succeed-or-nothing semantics). The dedup insert now writes `status: 'processing'`; on a `23505` unique-violation conflict, the route looks up the existing row's status — `'completed'` still short-circuits as a genuine duplicate, but `'processing'` or `'failed'` (or a vanished row) falls through and reclaims it, re-running the handler. All 16 `captureApiError(` call sites inside the event `switch` (spanning all 4 event types the route handles) were renamed to a local `captureAndFail` wrapper that logs via `captureApiError` *and* sets a `handlerFailed` flag; the whole switch is now wrapped in try/catch (catching thrown exceptions too); a finalize step after the switch writes `status: handlerFailed ? 'failed' : 'completed'` back onto the claim row. This is safe to apply to every switch-body site, not just the checkout-unlock path, because every downstream write in the switch is an idempotent `UPDATE ... WHERE id = ...` / `.eq(...)`, never an `INSERT` — reprocessing on redelivery cannot duplicate side effects. **Response codes are deliberately unchanged** — the route still always returns `200`/`{received: true}`, even on a `failed` claim; flipping that to `500` so Stripe actually redelivers is RS-2's job.

**Critical finding — the migration has NOT been applied to production.** Per the live-schema-is-truth rule, queried the live Supabase REST API directly: `GET /rest/v1/processed_webhook_events?select=status&limit=1` returned `{"code":"42703","message":"column processed_webhook_events.status does not exist"}`. (For comparison, `GET /rest/v1/applications?select=retention_hold_at&limit=1` returned `[{"retention_hold_at":null}]` — confirming RS-10's migration *is* live, unlike this one.) This is the same CLI/sandbox wall Session 146 hit on its own migration: no cached DB password, and this sandbox has no egress to the Postgres pooler on port 5432 regardless. **Do not deploy the already-committed route.ts code ahead of the migration** — it reads and writes `processed_webhook_events.status` unconditionally and would 42703 on every single Stripe webhook delivery if it went live first.

**Collision check against Session 146's concurrent Sprint DR work:** the sprint doc's own "Blocked on Romy" section carried a warning to confirm DR's current WIP state before touching `src/types/generation.ts` or the webhook route, since DR-1/DR-2 might have open edits there. This was only checked *after* route.ts had already been edited and committed in this session, not proactively beforehand — worth doing earlier next time a concurrent-session warning like this exists. The check itself found no actual collision: Session 146's two commits (`279e8be` on `docs/SPRINT_DR_DELIVERY_RELIABILITY.md`, `8b3df1c` on `src/types/generation.ts`) don't touch the webhook route, and `git status --short` — captured both at the start of this session and again after all 3 RS-1 commits — showed no uncommitted or untracked Session 146 changes to the webhook route at any point either.

### Commits landed this session

| # | Commit | File | What |
|---|---|---|---|
| 1 | `35570ae` | `supabase/migrations/20260910160000_webhook_dedup_status.sql` (new) | Adds `processed_webhook_events.status TEXT NOT NULL DEFAULT 'completed' CHECK (status IN ('processing','completed','failed'))` — **not yet applied live, see above** |
| 2 | `fd713c6` | `src/app/api/stripe/webhook/route.ts` | Claim lifecycle: insert-as-`processing`, reclaim-on-conflict-if-not-`completed`, `captureAndFail` flag tracking across all 16 switch-body error sites, try/catch around the switch, finalize-update to `completed`/`failed` |
| 3 | `048dc09` | `src/app/api/stripe/__tests__/webhook-dedup-claim.test.ts` (new) | 6 tests: clean completion, genuine-duplicate short-circuit, reclaim from `processing`, reclaim from `failed`, captured-error → `failed`, thrown exception → `failed` |
| 4 | `3157d13` | `docs/SPRINT_RS_RELIABILITY_SECURITY.md` | RS-1 task-table row and detailed section updated to `BLOCKED (needs Romy)` (code+tests done, migration pending — not DONE, unlike RS-10); new paragraph in "Blocked on Romy" spelling out the Dashboard SQL Editor fix |

### Next agent — start here

**Flag to Romy first:** `supabase/migrations/20260910160000_webhook_dedup_status.sql` needs to be run in the Supabase Dashboard SQL Editor (same manual-apply pattern as Session 146's still-blocked `generation_resume_log` migration) before RS-1's code can safely deploy. Once confirmed live (re-run the schema-drift check or repeat the REST probe above), RS-1 flips from `BLOCKED (needs Romy)` to `DONE`.

Per the sprint doc's own priority order, Phase 1 continues:
- **RS-2** — wrap the webhook event switch in an error boundary (Gap G-14). Substantially smaller now: RS-1 already added the try/catch and `handlerFailed` tracking RS-2 needs. Remaining scope: return `500` instead of `200` when `handlerFailed` is true (so Stripe's own redelivery logic actually retries), verify the `redis.del` calls are covered by the try/catch, and add `src/app/api/stripe/__tests__/webhook-error-boundary.test.ts`.
- **RS-3** — bind middleware Supabase errors, fail open with a Sentry alert (Gap G-15).
- **RS-4** — paid-but-locked-out reconciliation cron backstop (Gaps G-13/G-14/G-15) — note the project is on Vercel's Hobby plan, capped at once-daily crons (Session 146 constraint).

This session's 8 unpushed commits on `dev` (5 RS-10 `5a33e70`..`7fb7b85` + 4 RS-1/docs `35570ae`..`3157d13`) still need Romy's confirmation before pushing to `origin/dev`.

---


## Session 147 — RS-10 (Gap G-19) resolved: retention-cron extraction + three-email purge sequence (September 10, 2026)

**Branch:** dev. Five commits, one file each, `5a33e70`..`7fb7b85` — **not yet pushed**, per the one-push-per-logical-unit habit; confirm with Romy before pushing. `npx tsc --noEmit -p .` clean and full `npx jest` suite green at every commit (pre-commit hook).

**What RS-10 was:** two problems tracked under Gap G-19 — the retention-notice copy on Module 1 and the privacy policy had drifted apart on the actual day-count schedule, and the purge cron had no notice sequence at all (a user could lose uploaded files with zero warning and zero confirmation after the fact). Romy's decision (received 2026-09-10, recorded earlier in `docs/SPRINT_RS_RELIABILITY_SECURITY.md`): three emails — on-generation notice, a T-minus-3-day reminder with a confirm-to-keep link, and an on-completion confirmation — plus a `applications.retention_hold_at` column so a confirmed hold actually stops the purge.

**The build-breaking issue found and fixed this session:** the cron's business logic (`purgeExpiredFiles`, `sendRetentionReminders`, `sendRetentionCompletions`) lived directly in `src/app/api/cron/data-retention/route.ts`. Next.js auto-generates a typed-exports whitelist per route file (`.next/types/app/api/.../route.ts` — only `GET`/`POST`/`PATCH`/etc. and a few config exports are legal), so exporting those three functions directly from `route.ts` for testing threw `TS2344` under `npx tsc --noEmit -p .`. Fixed by extracting them (plus `daysAgo`, `BUCKET`, and the two file-age constants) into a new `src/lib/retention-cron.ts`, matching the codebase's existing convention of plain-module business logic (`retention-sequence.ts`, `partnership-hold.ts`) imported by thin route handlers.

### Commits landed this session

| # | Commit | File | What |
|---|---|---|---|
| 1 | `5a33e70` | `src/lib/retention-cron.ts` (new) | `purgeExpiredFiles`/`sendRetentionReminders`/`sendRetentionCompletions` + `daysAgo`/`BUCKET`, moved verbatim out of the route |
| 2 | `6fc5c50` | `src/app/api/cron/data-retention/route.ts` | Import the three functions from the new lib instead of defining them inline — fixes the `TS2344` typed-routes error |
| 3 | `f03e117` | `src/lib/emails/__tests__/retention-sequence.test.ts` (new) | 19 tests covering the three retention-email builders/senders (subject/body copy, singular/plural file counts, confirm-to-keep link HMAC round-trip, suppression-list skip) |
| 4 | `ba130b6` | `src/app/api/cron/__tests__/data-retention-hold.test.ts` (new) | 4 tests — `purgeExpiredFiles` correctly excludes only the held application's documents from a purge run; Module 1 and the privacy policy assert the identical 30/90-day schedule text |
| 5 | `7fb7b85` | `docs/SPRINT_RS_RELIABILITY_SECURITY.md` | RS-10 section rewritten RESOLVED/DONE (RS-7 style), task-table row flipped to DONE 2026-09-10, stale "Blocked on Romy" note replaced — nothing is currently blocked on this sprint |
...
- **RS-1** — turn the webhook dedup row into a claim, not a receipt (Gap G-13)
- **RS-2** — wrap the webhook event switch in an error boundary (Gap G-14)
- **RS-3** — bind middleware Supabase errors, fail open with an alert (Gap G-15)
- **RS-4** — paid-but-locked-out reconciliation cron (Gaps G-13/G-14/G-15)

Behind Phase 1: RS-5/RS-6 (Phase 2), RS-8/RS-9/RS-13 (Phase 3), RS-11 (Phase 4), RS-12 (Phase 5) — full detail in `docs/SPRINT_RS_RELIABILITY_SECURITY.md`. Confirm branch is `dev` before every commit; ask before pushing to `origin/dev`.

---

## Session 146 — Sprint DR Phase 1 (DR-1/DR-2/DR-7) started, blocked on migration apply (September 10, 2026)

**Branch:** dev. Two commits pushed-ready but **not yet pushed** (`279e8be`, `8b3df1c`) — awaiting the rest of the Phase 1 sequence before a push per the one-push-per-logical-unit habit of recent sessions; confirm with Romy before pushing. `npx tsc --noEmit` clean, `npx jest` 273 tests green at both commits (pre-commit hook).

**Scope decisions Romy made this session (via AskUserQuestion, not free text):**
1. Build DR-1/DR-2/DR-7 as one complete shippable unit first; DR-6 (release-with-flag + the "held for review" all-or-nothing gate fix) is a **separate follow-up session** — do not start it here.
2. When DR-6 is eventually built, also fix the pre-existing "held for review" quality-gate feature so it's partial-release-safe (currently blocks the entire document package on one failure) — noted for that future session, not scoped now.
3. Vercel plan is **Hobby (free)** — crons are capped at once/day. This **invalidates the original Sprint DR design's `*/15 * * * *` cron schedule** for the new `generation-resume` cron; it must run once daily, most usefully just before `health-watchdog`'s `0 3 * * *` (e.g. `50 2 * * *`) so it gets first crack at stale jobs before the watchdog force-fails them at its 30-minute-stale threshold.

**What DR-1/DR-2/DR-7 actually are** (full detail in `docs/SPRINT_DR_DELIVERY_RELIABILITY.md`):
- **DR-1 (checkpointed resume):** a new cron finds generation jobs stale past a threshold and resumes them, skipping documents already `approved`. Romy's condition for accepting this over a full durable queue: every resume attempt's outcome must be recorded in a queryable table (not just alerted), so a rising failure rate is visible from a query.
- **DR-2 (status vocabulary):** `/start` and `/run` currently check in-flight status with two different, divergent inline arrays (one includes dead values not in the live DB constraint). Replace both with one exported `IN_FLIGHT_STATUSES` constant.
- **DR-7 (approved-doc lookup bug):** `generation-engine.ts` (~line 2846) scopes the "already approved" lookup by `job_id` — but `/start` mints a new `job_id` on every retry, so this always returns empty on resume, defeating DR-1's entire cost-saving point. Fix: a new `application_id`-scoped helper, read-time "latest row per document_type" dedup (not a write-time upsert — an upsert has dangerous overwrite semantics for locked/approved docs).

**Design accepted from the Plan agent** (this session ran Plan Mode start-to-finish, then implementation began): dependency-injected `supabase` param on new lookup functions (matches the existing `partnership-hold.ts` / `partnership-hold.test.ts` fake-client pattern — this codebase does not use `jest.mock('@supabase/supabase-js')` anywhere), a dedicated `generation_resume_log` table (not reusing `document_generation_log`, which lacks a `job_id` column and can't represent a 3-state outcome), and an attempt cap (skip a job after ≥3 prior resume-log rows) so a permanently-broken job doesn't get retried forever.

### Commits landed this session

| # | Commit | File | What |
|---|---|---|---|
| 1 | `279e8be` | `docs/SPRINT_DR_DELIVERY_RELIABILITY.md` | Recorded Decision 2 (checkpointed resume + resume-telemetry requirement) and Decision 3 (release-with-flag) resolutions; updated DR-1 and DR-6 exit/test criteria |
| 2 | `8b3df1c` | `src/types/generation.ts` | Added `IN_FLIGHT_STATUSES`/`InFlightStatus` (after `GenerationJobStatus`, line ~52) and `ResumeOutcome`/`GenerationResumeLogEntry` (after `GenerationLogEntry`, line ~242) |

### The blocker — migration written, cannot apply it

`supabase/migrations/20260910130000_generation_resume_log.sql` exists on disk (untracked, not committed — see full contents below) but is **not applied to the live database**. It follows the exact RLS pattern of `20260630200000_rls_admin_log_tables.sql` (service-role-only: `ENABLE` + `FORCE ROW LEVEL SECURITY`, zero policies) and was schema-drift-audited clean before writing (`audit-schema-drift.py --refresh` showed zero drift; confirmed live FK targets `document_generation_jobs` and `applications` exist; confirmed `generation_resume_log` does not yet exist).

**Why it's stuck — two separate problems, confirmed by letting the hung `migration list` finish in the background:** (1) the CLI wants a password via `SUPABASE_DB_PASSWORD` env var, which isn't set anywhere in `.env.local` or `supabase/.temp/`; (2) even independent of the password, the actual failure was a **network timeout** — `dial tcp 3.148.140.216:5432: connect: operation timed out` — this sandbox cannot reach the DB pooler on port 5432 at all. So this isn't just "no cached password," it's that CLI-based `migration list`/`db push` may not be reachable from this environment full stop, password or not (consistent with the Session 142 note that this sandbox also has no egress to Upstash Redis). **The Dashboard SQL Editor isn't just the fastest path here — it may be the only one available to a non-interactive session.** No `exec_sql`-style RPC exists in the codebase to push DDL through the service-role key over PostgREST instead. Sessions 138 and 139 hit the same wall and resolved it by **Romy pasting the SQL into the Supabase Dashboard SQL Editor by hand.**

**Why nothing after this was built:** the project's live-schema-is-truth rule means writing `src/lib/generated-documents-lookup.ts`, the cron route, or their tests against a table that only exists as an unapplied `.sql` file would itself violate that rule. Everything past commit 2 is designed but intentionally not written yet.

**The migration SQL to run** (Supabase Dashboard → SQL Editor → paste → Run, then tell the next session it's done so `audit-schema-drift.py --refresh` can confirm and the file can be committed):
...
1. Confirm with Romy whether the migration above has been run in the Dashboard SQL Editor. If not, ask her to run it (or supply the DB password to the CLI) before writing any more code.
2. Once live, re-run the schema-drift audit to confirm, then commit `supabase/migrations/20260910130000_generation_resume_log.sql` alone (commit 3).
3. Continue the planned sequence, each its own commit: `src/lib/generated-documents-lookup.ts` (new — `getLatestDocumentsForApplication`/`getApprovedDocumentsForApplication`, DI seam) → `src/lib/__tests__/generation-resume.test.ts` (lookup tests, fake-client pattern) → `src/lib/generation-engine.ts` ~line 2846 swapped to the new helper (**highest-risk commit — run the full suite carefully**) → same swap in `src/lib/cic-package-manifest.ts` (~line 118) and `src/app/api/generate/download/[applicationId]/route.ts` (~line 137) → `src/app/api/generate/start/route.ts` (line ~91) and `src/app/api/generate/run/[jobId]/route.ts` (line ~66) switched to `IN_FLIGHT_STATUSES` → `src/app/api/generate/__tests__/job-status-vocabulary.test.ts` (new, DR-2's named test) → `src/app/generate/[applicationId]/page.tsx` (~lines 353-373, always re-issue `/run` on reattach instead of branching) → `src/app/api/generate/progress/[jobId]/route.ts` (~line 110, add `'partial'` to the SSE terminal-status check) → `src/app/api/cron/generation-resume/route.ts` (new — the cron itself, Bearer `CRON_SECRET` auth like `health-watchdog`, extracted `resumeStaleJob(supabase, job)` for testability, attempt cap at ≥3 prior log rows) → extend `generation-resume.test.ts` with cron-orchestration tests → `vercel.json` (add the cron entry at **`"50 2 * * *"`**, once daily per the confirmed Hobby plan — do not use the sprint doc's original `*/15 * * * *`).
4. After all commits: stop the dev server, run `npm run build` clean.
5. Demonstrate Exit criteria for real (not just reasoned about): start a real job, back-date `updated_at` via SQL (or a deliberate kill) to force staleness, invoke the cron directly with `CRON_SECRET`, confirm previously-approved documents are untouched while only un-approved ones regenerate, confirm exactly one correctly-labeled `generation_resume_log` row, confirm the browser reattach path resumes correctly.
6. Confirm branch is `dev` before every commit. Ask before pushing to `origin/dev`.

**Loose end, not this session's scope:** the Session 145 handoff flagged an uncommitted 14-file brand-casing sweep (`e2go` → `E2go.app`) as needing Romy's confirmation before committing/reverting. Current `git status` on `main` shows only `CLAUDE.md` modified (uncommitted) — the brand-casing sweep itself appears to have already landed via the `4f88aa3` merge to `main` per the repo's recent commit history. Worth a quick confirmation with Romy, not urgent enough to interrupt Sprint DR work.
...
`results/page.tsx` already had a complete "Contact us for partnership pricing" branch — price card, explanatory note, `mailto:` CTA, add-on suppression — gated on `data.application_type === "complete_partnership"`. **The quiz writes `partnership` / `spousal_partnership`.** `complete_partnership` is a legacy *payment_type*, never an *application_type*. Live data confirms it appears nowhere: `quiz_sessions.application_type` = `{solo: 12, partnership: 1}`, `applications.application_type` = `{solo: 4, partnership: 1}`.

So the branch was unreachable, and **every partnership applicant to date saw the solo $990 price and a live checkout button** — while the pipeline gates all six `*_p2` documents on the unpurchasable `complete_partnership` payment type. A two-investor client could pay and receive a package in which their partner does not exist.

Closed at three layers, server-side authoritative:

| Layer | File | Commit |
|---|---|---|
| Shared helper — `resolvePartnershipHold()`, application-type vocabulary, hold copy | `src/lib/partnership-hold.ts` | `5cd3dc5` |
| 10 unit tests — both lookup paths, fail-open, hold-survives-partial-error | `src/lib/__tests__/partnership-hold.test.ts` | `9657775` |
...
The guard reads the newest `quiz_sessions` row first, then the newest `applications` row, and returns **409 + `partnershipHold: true`** *before* any `applications` row is created and *before* the Stripe session. It **fails open by design** — a transient DB error must never block a paying *solo* applicant — and surfaces `lookupError` so the caller reports it to Sentry. Add-ons (`fdd_analysis_addon`, `market_analysis_addon`, `simulator_3pack`) and `renewal` sit outside `PACKAGE_TIER_IDS` and are not held.

`partnership-hold.ts`'s doc comment couples its own removal to building the tier, so the hold cannot be lifted without the Stripe Price IDs existing.

### 2. The Canada labels — the reported gap named two, the sweep found eleven files

`QUESTION_LABELS` is serialised straight into the generation prompt (`generation-engine.ts:25` imports it; line 1117 emits `Q: … / A: …`), so a Canadian premise in a *label* becomes a false premise in every non-Canadian applicant's prompt. Two findings were structural rather than cosmetic:

- **`M3-A-05` offered Canada as its only citizenship option**, rendering live in Tab A — a non-Canadian applicant literally could not state their nationality. Worse, the option value `"CA"` could never match a prefilled value: `getPreFill()` returns `answers["Q0-01"]`, a full country **name**, and `FormField.tsx`'s plain `<select>` renders blank on an unmatched value. Fixed by sourcing options from `TREATY_COUNTRIES` (value === label === country name). The dormant `"CA"`-keyed `warningTriggers` array — never mapped into `FieldConfig` — went with it.
- **`src/data/module3/tab-f.json` has no consumers.** The live Tab F is hardcoded in `f/page.tsx` and the two had already drifted (different QF-04 text). But `scripts/generate-question-registry.mjs` still reads `tab-*.json`, so **the dead file's Canadian labels were still reaching the LLM prompt.** Both fixed.

13 commits, one file each: `99b03e3` tab-a.json · `321f7dd` a/page.tsx · `36e8494` tab-f.json · `d47a1bf` f/page.tsx · `1449744` investment/page.tsx · `6401aa3` partner2/page.tsx · `8636455` business/page.tsx · `1980dbd` module1/page.tsx · `4749490` module3/b/page.tsx · `a6693f8` renewal/intake/page.tsx · `b3183aa` RenewalEntryClient.tsx · `ac59911` renewal/documents/page.tsx · `364ab25` question-registry.generated.ts.

The regenerated registry changed **exactly the two originally-reported lines** — `M3-A-09` "Current home address in Canada" → "…in your country of residence", and `QF-06` "…leaving your Canadian account?" → "…leaving your home-country account?" — which confirms the other eleven files were fixed without collateral drift.

**Five items deliberately deferred, each with a reason recorded** (now DR-11…DR-15 and DR-19): the Toronto-only consulate list at `apply/qualifications/page.tsx:167` (needs a real E-2 consulate list from Romy — inventing one puts wrong consulates in front of clients); the franchise archetype prompt block at `generation-engine.ts:246–247` (Canadian ties, RRSP/TFSA, "would return to Canada", "traceable from the Canadian source account"); `interview-knowledge-base.ts` lines 314/334/343/428/435/439 plus `api/simulator/interview-prep/route.ts:185–186` (coaching content needing domain review); the `financial_assets_portfolio` trigger keyed only on `rrsp`/`tfsa`/`lira`/`crypto`, so a French applicant funding from a securities account **never gets that document at all**; and `PricingClient.tsx:208` hardcoding `application_type: 'solo'` (nothing leaks — the server guard reads `quiz_sessions` first — but the row is wrong in the database).

### 3 & 4. The documentation and the sprint

Two new docs, both indexed in `docs/DOC_INDEX.md`:

- **`docs/DELIVERY_RELIABILITY_GAPS.md`** — owns *what is broken*. The traced payment→generation→assembly→download failure path step by step with the file producing each step; gaps **G-01…G-12** with severity, root cause and `file:line` evidence; what the client actually receives (the 29-file ZIP manifest and the typography); and the fee-vs-delivery reconciliation of the Foundation feature list against what the engine really produces. G-03 and G-09 are marked with this session's commit hashes.
- **`docs/SPRINT_DR_DELIVERY_RELIABILITY.md`** — owns *what we do about it*. Tasks **DR-1…DR-22** across 8 phases, each with kind, estimate, an exit criterion that is a demonstration, and a **named automated test**. The sprint's own definition of bullet proof: *no task is Done on reasoning; a task whose test does not exist is not Done.*

**The invariant the sprint installs:** a job in flight always has something that will finish it or fail it, and every failure produces — within 15 minutes — a client-visible message with a next action and a signal we can see without being asked.

**Effort:** ~22–23 eng-days, ~10 of Romy's hours, $200–400 LLM. **Phases 1–2 (6–7 days) are the launch gate.**

### The headline finding, unchanged

The engine produces good documents; the **delivery system around it** is where a paying client gets hurt. The pipeline is fired without being awaited on a platform that reclaims the instance once the response is sent — so a killed run never *fails*, it stops with the row still reading `running`; `/start` then refuses to mint a new job while one is `queued`/`running`, and the client only calls `/run` for new jobs, so **reloading makes it permanently worse**. Nothing emails anyone, nothing alerts us, and the only escape is a watchdog that runs once a day at 03:00 UTC — and never at all if the job stalled at `queued`. **It is not a crash loop; it is a silent stall with a self-closing trap door.**

### Pushed

Romy gave the go-ahead on September 10, 2026. All 22 commits went to `origin/dev` as `28bc75e..340a6c8`. The pre-push hook ran the full gate — `npx tsc --noEmit`, `npm test` (19 suites / 273 tests), `npm run build`, and `npx playwright test` (27 security specs) — all clean. Nothing was pushed to `main`.

A concurrent brand-capitalisation edit (`e2go` → `E2go.app`) appeared in the working tree during the push, touching 14 files across `src/app/` and `src/lib/emails/`. It is **not** part of Session 145 and was left uncommitted.

### Blocked on Romy

| Needed | Blocks |
|---|---|
| **Decision 2** — durable execution shape (queue / checkpointed resume / `waitUntil()` + segmentation). Recommendation: checkpointed resume now. | DR-1, the top of the sprint |
| **Decision 3** — partial-package policy: 19 of 20 succeeded, hold or release-with-flag? Recommendation: hold and notify while volume is low. | DR-6 |
| Real E-2 consulate list for `M3-I-11` | DR-11 |
| Domain review of the six Canada-assuming coaching passages | DR-13 |
| Partnership tier pricing, so the Stripe Price IDs can be created | DR-18 |

---

## Session 144 — `cf8b44f` recorded, and the Stage 5 `elementPatterns` gap closed (September 10, 2026)

**Branch:** dev. Two commits, `5183af8`, `7bd2253` — **not yet pushed**, pending Romy's go-ahead. `npx tsc --noEmit` clean; `npx jest` 18/18 suites, 263/263 tests; `npm run build` clean.

### `cf8b44f` — the early-access discovery, resolved
Session 143's "unrelated discovery, not mine, not committed" (the uncommitted `await` on the welcome-email send in `src/app/api/early-access/route.ts`) was **Romy's own change, and Romy committed it** as `cf8b44f` "Await the early-access welcome email send" — now on `origin/dev`. Its message records the production evidence: a real submission wrote its `early_access_leads` row but never reached Resend at all (no send attempt in Resend's log for that address) — the fire-and-forget call had no `waitUntil()`/`after()` keeping it alive, so Vercel tore the function down before the un-awaited send got out. Nothing outstanding on this item; the Session 143 "get a decision on it" priority is closed.

**Also corrected:** Session 143's entry claimed its four commits were "not yet pushed." All four (`b30a8f4`, `2376e29`, `b846bf3`, `6f8ca3c`) are on `origin/dev`. Only this session's two are unpushed.

### The `elementPatterns` gap — was a silent, unconditional delivery blocker
Session 143 flagged this and deferred it ("the Stage 5 `elementPatterns` map is missing entries for several other document types' required elements too … those documents' required-elements checks likely fail unconditionally regardless of content"). Measured: the map covered **24 of the 45** element names in use. The other 21 resolved to `[]`, so `patterns.some(...)` was always `false` and the element was reported missing no matter what the document said.

This was not a cosmetic warning. A Stage 5 miss writes `quality_gate_passed: false`, which Session 142's Gap 3 gate renders as a blocking `'blocked'` / "HELD FOR E2GO REVIEW" status across the manifest, certify and download paths, with `packageReady: false`. **Eight document types had zero covered elements and could therefore never be delivered**, regardless of LLM output quality:

| Document type | Uncovered required elements |
|---|---|
| `visa_category` | 4/4 — investment_substantiality, proportionality_ratio, total_enterprise_cost, committed_vs_reserved |
| `nonimmigrant_intent` | 3/3 — home_country_ties, return_intent, departure_plan |
| `nonimmigrant_intent_p2` | 2/2 — home_country_ties, return_intent |
| `marginality_rebuttal` | 3/3 — employment_projections, market_evidence, revenue_projections |
| `net_worth_statement` | 3/3 — total_assets, total_liabilities, net_worth |
| `property_portfolio` | 3/3 — property_location, ownership, estimated_value |
| `declaration_spouse` | 2/2 — spouse_name, derivative_status |
| `gift_letter` | 3/3 — donor_name, gift_amount, irrevocability |
| `resume_spouse` | 1/2 — spouse_name |

### The fix
- **`REQUIRED_ELEMENT_PATTERNS` hoisted to module scope** with all 21 missing entries added, **calibrated against the 15 real documents from the Session 143 live run** (`test-uk@example.com`, James Windsor) rather than invented examples.
- **`findMissingRequiredElements()` extracted**, and it **fails open** on an element with no pattern. A future `REQUIRED_ELEMENTS` addition can now only cause a *missed* check, never an unconditional block — false negatives are recoverable downstream, false positives block delivery outright, which is the failure mode that shipped.
- **A CI drift test is what makes fail-open safe**: every element named in `REQUIRED_ELEMENTS` must have a pattern, asserted in jest, so an uncovered element is caught at build time instead of silently skipping a check at runtime.
- **`applicant_name` now matches the real name** from the case brief when it is available. The original `/applicant/i` proxy was blocking `org_chart`, which names James Windsor **eight times** and never uses the word "applicant" — one of the 6 documents held in the Session 143 live run, and a pre-existing bug in the *original* 24 patterns, not in the new ones.

### Verification
Validated the finished map against the real Session 143 output: **45/45 elements covered**; all 15 real documents pass (gift_letter correctly skipped as the N/A sentinel); and a junk-content negative control still flags **every** element on all eight previously-uncovered types — permissive enough for real output, still strict enough to catch an empty one.

### Commit sequence (2, one file each, `dev`)
`5183af8` `src/lib/generation-engine.ts` (patterns + fail-open helper + Stage 5 rewire) · `7bd2253` `src/lib/__tests__/generation-engine.test.ts` (coverage drift test + 12 regression cases).

### Process note — commits initially landed on `main`
The working tree was left on `main` (a `dev` → `main` checkout + merge, `502755b`, already pushed) rather than `dev`, and both commits were made there before the branch was checked. Corrected in-session: cherry-picked onto `dev`, `main` hard-reset to `origin/main` (nothing of `main`'s own was lost — `502755b` was already on the remote). **Confirm the branch before committing, not after.**

### Next priorities
- Get Romy's go-ahead to push `5183af8`, `7bd2253` to `origin/dev`.
- Still deferred from Session 143: page-length overages, missing legal disclaimers on 5 documents, Cover Letter's missing "direct and develop" phrase, Business Plan brevity.
- The unimplemented `gift_letter` "omit from package when NOT APPLICABLE" promise — still not built.

---

## Session 143 — First real generation job: verified Gaps 1–3 live, fixed two bugs it exposed (September 9, 2026)

**Branch:** dev. Four commits, `b30a8f4`, `2376e29`, `b846bf3`, `6f8ca3c` — ~~not yet pushed~~ **all four are now on `origin/dev`** (corrected Session 144). `npx jest` 18/18 suites green (pre-commit hook, every commit); `npm run build` clean.
...
### Bug found & fixed — case brief missing `applicant_name` / `business_name`
The case-brief JSON for this persona didn't carry `applicant_name` or `business_name`, which several prompts read directly. `runGenerationPipeline()` (`src/lib/generation-engine.ts`) now backfills, right after the case-brief row loads:
- `applicant_name` / `principal_name` / `business_name` / `business_category` from the `applications` row (only when the case-brief JSON doesn't already have them), and
- `investment` from the `M3-F-02` answer (parsed as a number) when the case brief has none.

Committed in `b30a8f4`.

### Bug found & fixed — quality-gate retry had no case data to regenerate from
The retry path that fires when `runQualityGate()` fails a document was calling the raw Anthropic SDK (`getAnthropic().messages.create()`) directly with just the system prompt and the literal user message `"Regenerate the document now."` — no case brief, no Module 3 answers, no investment breakdown, no exhibit registry. Claude correctly had nothing to regenerate from and refused, and **that refusal text was what got persisted as `content_text`** — meaning a document that failed the gate could come back worse, not better. Fixed by routing the retry through `callClaudeAPI(payload: GenerationPayload)` — the same function every other regeneration site in the file already uses — building a `retryPayload` that prepends the gate's `failureInstructions` onto `case_theory_brief` so the model has both the full case context and what to fix. Committed in `b30a8f4`, alongside the case-brief fix (same file, one commit).

### Bug found & fixed — `.env.local` quote-stripping in the persona-seed script
`scripts/run-persona-generation.mjs`'s `.env.local` line parser kept surrounding quotes on quoted values, breaking anything downstream that compared the raw string. One-line fix: strip a leading/trailing `"` or `'` after the existing `key=value` regex match. Committed in `2376e29`.

### Gap 3 verified live — blocked-status delivery gate, both layers
- **API layer:** `GET /api/dashboard/package-manifest` — this route authenticates via `Authorization: Bearer <token>` header, not the cookie session most routes use. Called with a properly minted session token for the test account: `200`, `packageReady:false`, 6 of 15 tabs `status:"blocked"` each with a specific `blockedReason` string.
- **UI layer:** the real `/documents/[applicationId]` page, viewed by minting an admin `generate_link` (magiclink) session for `test-uk@example.com` and injecting it into the browser via `window.__e2go_supabase__.auth.setSession({access_token, refresh_token})` (no password touched) — correctly renders a red "HELD FOR E2GO REVIEW" badge with the specific WARNING/CRITICAL reasons and a "REQUEST NEW DRAFT" button for every blocked document, vs. "AWAITING CERTIFICATION" + "READ & CERTIFY" for passing ones. This closes out the last unverified piece of Session 142's Gap 3 work.

### Gap 1 verified live
Voice/person correctness confirmed directly in the generated output: principal and spouse declarations are first-person, qualifications/résumés are third-person, cover letter is officer-facing — matching the per-doc "VOICE & PERSON" deltas from Session 142's extraction.

### New finding — placeholder leak bypasses the Gap 3 gate (not fixed, needs a decision)
Several documents (Cover Letter, Non-immigrant Intent Statement, Fund Flow Chronology, Consolidated Net Worth Statement, DS-156E/DS-160 Reference, Principal Applicant Résumé) contain literal unfilled template markers — `[Date]`, `[Consulate address]`, `[DATE REQUIRED]`, `[AMOUNT NOT PROVIDED]`, `[NO SUPPORTING EXHIBIT ON FILE]`, `[CONFIRM WITH APPLICANT]`, `[Phone on file]`, `[Email on file]`, `[Date of birth on file with application]`. The UI surfaces these as separate "CRITICAL" issues — but that check runs **independently of `quality_gate_passed`**, so a document can carry leaked placeholders and still show "AWAITING CERTIFICATION," eligible for client certification. Confirmed concretely on the Principal Applicant Résumé: 3 unfilled placeholders, `quality_gate_passed` not false, certifiable as-is. This is a real hole in the "no document that fails review reaches the client unreviewed" goal from Session 142 — it needs the same kind of blocking treatment Gap 3 gave the legal-boundary check, or an explicit decision to defer it.

### Other quality-gate calibration findings from the real run (not fixed — deferred)
- `gift_letter` false positive: correctly outputs "NOT APPLICABLE — No gift funds used in this investment" but is still held for "Missing required elements: donor_name, gift_amount, irrevocability" — the gate doesn't recognize a valid N/A response as satisfying the requirement.
- Page-length overages on 7+ documents, missing legal disclaimers on 5, missing required structured elements on 6+, Cover Letter missing the required "direct and develop" USCIS phrase, Business Plan flagged as too brief.
- These are genuine first-real-run QA signal on gate calibration, not regressions from this session's two fixes — flagged for Romy to prioritize separately.

### Operational note — `.next` build/dev cache collision, second occurrence
Hit the same corruption as before (`Cannot find module './9085.js'` etc.) mid-session — root cause this time was a forgotten lingering background `npm run build` process from earlier in the session, still writing to `.next` while the dev server served requests. Same fix (`preview_stop` → `rm -rf .next` → `preview_start`, confirmed via `/api/health` + a real page route before touching the browser again); new safeguard applied going forward: stop the dev server before running any further production build in a session that also has one running.

### Verification
- `npx jest --silent` — 18 suites / 247 tests, twice (once per commit's pre-commit hook).
- `npm run build` — clean.
- Manual E2E: full 15-doc generation job; direct API call to `package-manifest` with a real bearer token; live UI walkthrough of `/documents/[applicationId]` as the test persona.
- Raw generated text for all 15 documents saved to `scratchpad/generated_docs_86250f1e/` for reference.
...
## Session 142 — Shared rule book + four document-generation gap closures (September 9, 2026)

**Branch:** dev. **Pushed** — 13 commits `258167e`..`959b2ba` on `origin/dev`, pre-push hook green (jest 247 + `next build` + 27 Playwright security tests, 48.1s). `npm run build` clean (192/192). Dev server restarted per session hygiene.

**Context:** Romy — *"extract the universal prompt into one shared file, and close the other gaps to make it efficient and then do regression testing"* + the standing *"test everything extensively before declaring it done, committing it, pushing it."* The platform's document-generation "rule book" has three layers: Layer 1 = `prompts/v1/documents/*.md` read by `loadPrompt()`; Layer 2 = the deterministic shared spine `buildGenerationPayload()`; Layer 3 = the post-generation quality gate → `runQualityGate()` → acknowledgment gate → `final_status`. Plan file: `~/.claude/plans/snazzy-prancing-wind.md`.

### Gap 1 — Universal system prompt extracted (was copy-pasted into 21 files, already drifted)
- **New `prompts/v1/_universal_system_prompt.md`** (106 lines) — canonical role statement + 8 core principles (SPECIFIC OVER GENERIC / FACTS ONLY — NO LEGAL CONCLUSIONS / ACTIVE VOICE / CREATIVE BUT HONEST / MATCH THE VOICE PROFILE / HUMAN NOT AI / CITE THE RECORD / LEGAL BOUNDARY — NEVER CROSS THIS LINE). Typo-free (the `until it until it` from `business_plan.md` is gone).
- **`src/lib/generation-engine.ts`** — `loadUniversalCore()` (memoized), `stripLegacyUniversalSection()` (defensive — a stale in-file `## UNIVERSAL SYSTEM PROMPT` can never double up), `loadPrompt()` returns `${core}\n\n${docBody}`. The 3 already-canonical files stay equivalent; the other 18 **gain** the full principle set (intended upgrade).
- **21 `prompts/v1/documents/*.md`** — stripped of the duplicated block; 15 kept a short `## DOCUMENT VOICE & FRAMING` delta (person/voice only), 6 were pure deletion.
- **New `src/lib/__tests__/universal-prompt.test.ts`** — fails if the shared core loses a principle heading, any doc file reintroduces `## UNIVERSAL SYSTEM PROMPT`, the typo comes back, or `loadPrompt()` stops assembling core + delta.

### Gap 2 — Rule-book version stamp (no schema change)
- `export const RULEBOOK_VERSION = '2026-09-09'` (bump by hand when a prompt or gate rule changes) + `getPromptCorpusHash()` (memoized sha256 over sorted `prompts/v1/**/*.md`).
- `runGenerationPipeline()` writes one `document_generation_log` row per job: `stage: 'rulebook_version'`, `flagged_sections: ['corpus:<hash>']`, `notes: JSON.stringify({ rulebook_version, prompt_corpus_hash })`. Columns match existing inserts — no migration.

### Gap 3 — Legal-boundary gate now blocks delivery (was detect-and-flag only)
- **(a)** `LEGAL_CONCLUSION_HARD_RE` rewritten to `/gi` word-boundary matching + a 48-char preceding-context negation guard — no longer fires on "the business is eligible for SBA 7(a) financing" / "the property qualifies for the capital-gains exemption".
- **(b)** `quality_gate_passed === false` is now a **blocking** status end to end:
  - `cic-package-manifest.ts` — new `'blocked'` `ManifestTabStatus` + `blockedReason` + `blockedCount`; `packageReady` is false while any tab is blocked.
  - `certify-document` route — 409 `{ held: true, reason }` when the doc failed the gate.
  - `generate/download/[applicationId]` route — blocked docs listed in the 403, excluded from the `uncertified` filter.
  - `documents/[applicationId]/page.tsx` — red "HELD FOR E2GO REVIEW" badge; certify button becomes a 3-way `held ? … : isCertified ? … : …`.

### Gap 4 — Per-source fund amounts wired into `fund_sources_sum_mismatch`
- **`src/app/apply/investment/page.tsx`** — `M3-F-05` stays a `multi` select (≈9 downstream readers depend on the `JSON.stringify(array)` shape). Additive: when ≥1 source is selected, one `<CurrencyInput>` per source renders under it, persisted via the existing debounced `handleAnswerChange` to companion key **`M3-F-05-AMOUNTS`** = JSON object `{ "<sourceKey>": <number> }`. Live reconciliation line vs `M3-F-02` (±$1 tolerance).
- **`src/lib/field-registry.ts`** — `['M3-F-05-AMOUNTS', 'Amount per fund source', false]`.
- **`src/lib/pre-generation-validation.ts`** — tolerant parse (object or JSON string) of `M3-F-05-AMOUNTS`; `sources` map gets real `amount` values; `sourcesMatchTotal` evaluates for real when amounts are present, still `true` when none are filled (unchanged behaviour for legacy apps / mid-flow saves).
- **`src/lib/__tests__/pre-generation-validation.test.ts`** — fixture gains `M3-F-05-AMOUNTS`; new describe block, 4 cases (mismatch → `fund_sources_sum_mismatch` gap; within $1; skip when key deleted; already-parsed object).
- **`src/lib/cpu-risk-signals.ts`** — comment-only: D4 header now points at the new key, notes dollar-weighting is "now possible but out of scope here"; D5/D6 still confirmed-blocked (no per-deposit dates).

### Verification
- `npx jest --silent` → **18 suites / 247 tests pass** (new: `universal-prompt.test.ts`; updated: `pre-generation-validation.test.ts`). Pre-commit hook re-ran the full suite on each of the 13 commits.
- `npm run build` → "✓ Compiled successfully" / "✓ Generating static pages (192/192)".
- **Manual E2E** on `test-france@example.com` (re-seeded via `node scripts/seed-test-profiles.mjs`): Module 3 → Investment → Source of funds — selected 2 sources, entered amounts that don't sum to `M3-F-02`, confirmed the reconciliation message shows the mismatch, corrected the amounts, confirmed it clears. Persistence check via direct Supabase query (using the correct `question_key` / `answer_value` columns) confirmed `M3-F-05` = `["savings","property-sale"]` and `M3-F-05-AMOUNTS` = a JSON object.
- Gap 2 verified by direct `document_generation_log` insert → 201. Gap 3b verified the `generated_documents.quality_gate_passed` / `quality_gate_notes` columns exist → 200.
- **Not done:** a full LLM generation job end-to-end. The sandbox has no external egress to Upstash Redis (`desired-leopard-67358.upstash.io` → ENOTFOUND), so routes that hard-await a redis call 500 locally regardless of the DB write. Judged adequately covered by 247 passing tests + clean build + the direct schema checks. Worth a real generation run in an environment with Redis before this ships to prod.

### Commit sequence (13, one file each, `dev`)
`258167e` `_universal_system_prompt.md` (new) · `da00771` strip duplicated block from 21 doc templates · `143fc3f` `generation-engine.ts` (assemble core + version stamp + corpus hash + audit row + regex tighten) · `9287607` `universal-prompt.test.ts` (new) · `c7c3bd4` `cic-package-manifest.ts` (blocked status) · `9a58ef7` `certify-document` route · `539451f` `generate/download` route · `405649a` `documents/[applicationId]/page.tsx` · `82d4940` `apply/investment/page.tsx` (per-source amounts) · `6fa0129` `field-registry.ts` · `e7b4720` `pre-generation-validation.ts` · `642efed` `pre-generation-validation.test.ts` · `959b2ba` `cpu-risk-signals.ts` (comment).

### Next priorities
- Run one real end-to-end generation job (with Redis reachable) to confirm the `rulebook_version` log row and the blocked-status path behave live.
- Bump `RULEBOOK_VERSION` on the next prompt/gate change (it's a manual constant by design).
...
### Sprint EM — Email System Audit (September 17, 2026)

Full audit of every email the app can send, requested by Romy after noticing
uncommitted local edits to `src/lib/emails/*`. A general-purpose agent grep'd
every call site (not just filenames) across `src/` and traced each send
function back to its actual trigger. Full inventory, content, triggers, and
fix tasks in `docs/SPRINT_EM_EMAIL_AUDIT.md`.

**Findings: 22 distinct emails.**
- 15 live and firing correctly (early-access welcome, quiz results, 3×
  quiz-nurture, package-ready, reset-after-failure, 3× retention, account-
  deletion notice, support ticket, partner invite, broker-request,
  franchise-referral).
- 7 dead:
  - Clock-1 inactivity sequence (Day 60/67/74/81, 4 emails) — gated on
    `EMAIL_SCHEDULER_ACTIVATED_AT`, confirmed NOT set in local `.env.local`;
    production not yet checked.
  - Clock-2 post-outcome sequence (Immediate + Day 60 + Day 83, 3 emails) —
    root cause: `sendOutcomeEmails()` has zero callers anywhere in `src/`;
    `src/app/api/dashboard/outcome/route.ts` never references email.

**Sprint EM created** (`docs/SPRINT_EM_EMAIL_AUDIT.md`) — tasks EM-1
(confirm prod env before touching anything) → EM-2 (activate Clock-1,
conditional on EM-1) → EM-3 (wire `sendOutcomeEmails` into the outcome
route, activates all of Clock-2) → EM-4 (optional Resend-client
centralization cleanup, out of scope unless requested).

**Status:** documentation only this session — no code changes made yet.
