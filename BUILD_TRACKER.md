...
**Last Updated:** July 3, 2026 — Session 119r: **WS6.1 three new documents.** Built Org Chart / Management Structure Exhibit, Corporate Documents Guide, and Lease/Premises Summary — all three wired end-to-end (DocumentType, every per-doc map in generation-engine.ts, tab/manifest constants, both conditionalDocTypes computations). Along the way, fixed two pre-existing bugs: `financial_assets_portfolio` was missing from `DOC_TYPE_TAB_MAP`/`DOC_DISPLAY_NAMES` entirely, and the pipeline executor's own `conditionalDocTypes` block never got the `financial_assets_portfolio` trigger added to `start/route.ts` in Session 119o — meaning that document was silently never generated. Build clean, `tsc --noEmit` clean, 150/150 tests. Commit `2c4a4f4`.
